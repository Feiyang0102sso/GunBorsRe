...
> 2026-09-10 玩家死亡演出修复：原 BIG 死亡动画与 76/256 慢动作继续播放，收到原脚本完成通知后才结算；新增 `stsuicide` 和永久研究入口80。四图致死/作弊/重开与相关回归见[本轮记录](docs/player-death-fix-2026-09-10.md)。

> 2026-09-10 地图遮挡修复：建筑、玩家、兄弟和敌人恢复共用主层Y排序，上侧遮挡、下侧可见，保留精灵透明区域。四图像素／实战与Boss、空袭回归通过；永久研究入口79，见 [修复记录](docs/map-occlusion-fix-2026-09-10.md)。

> 2026-09-10 击杀 XP 浮字、Powerup 模式外框及 Airstrike 外围动画已恢复：补回原文字效果，纠正旋转精灵多翻转，恢复选择器主 Movie 的保留／退场。原版依据、像素红绿回归及验证边界见 [本轮记录](docs/xp-ui-decoration-fix-2026-09-10.md)。

> 2026-09-10 Boss/道具追加修复：敌人光束保持发射方向，补齐原电弧，移除爆炸多乘的熟练度暴击；左右道具待机、装备保存及紧急购买拖动已修复。REV10倍率核查正常，pack12破甲后一雷死亡是原Flow规则。依据及回归见 [本轮记录](docs/boss-powerup-fixes-2026-09-10.md)。

> 2026-09-10 `stboss` 已实现：后台完成本波原刷怪/死亡回调后立即进入 Boss 演出，无需玩家继续清怪；四图 20 个波次及 SDL 输入检查通过，见 [作弊码记录](docs/stboss-2026-09-10.md)。

> 2026-09-10 Boss 修复：恢复原生镜头目标、排除手雷直接接触误拆甲；四图入场、原装甲分支及死亡回普通波专项通过，并纠正旧研究“冰雷不拆甲”的结论。见 [Boss 恢复记录](docs/boss-restoration-2026-09-10.md)。

> 2026-09-10 紧急购买 USE NOW 恢复逐行居中；三种空袭补齐关卡暂停/恢复和选择器警示入口，六条实际 Session 路径及 Release 回归通过，见[空袭与文字修复](docs/airstrike-selector-fix-2026-09-10.md)。

> 2026-09-10 音频交接修复：商店按原动作帧播放切枪WAV；音乐跨菜单/加载/战斗/结算由外层流程持有，结算结束进入精炼厂才切回菜单曲。真实SDL零音量队列及相关回归见[音频修复记录](docs/audio-transitions-fix-2026-09-10.md)。

> 2026-09-10 武器修复：激光恢复零宽射线与近距离端点；按原 BULLET Flow 恢复 Ribbon，修正默认步枪颜色和 Kraken 导弹细尾迹；商店筛选不再隐藏切枪按钮。来源、红绿回归及边界见[武器修复记录](docs/weapon-effects-fix-2026-09-10.md)。

> 2026-09-09 追加修复：Haven 问号按原对象登记顺序绑定 Spire 实例；商店分类和星球→REV 横扫修复冷加载跳过；Logo 直接交接 Splash，右下角原小人加载，不插入 CG。来源及回归见 [UI 修复追加记录](docs/ui-restoration-2026-09-09.md#追加修复spire横扫与启动交接)。
...
- [x] **UI4.5 升级不足币、换枪和卡内熟练度。** 原两按钮动态总价/差额提示、最小足额BIG内购、等待后重试升级、切换另一把未满级装备枪；原账户隔离回归及重载退出0。卡内Movie91使用原99比例和固定星Sprite初帧，210枪阈值及章节变异通过。2026-09-09证据见[本轮记录](docs/ui-original-overnight-2026-09-09.md)。普通商店三按钮不足币提示在后续[UI清理](docs/ui-restoration-2026-09-09.md)中恢复。
...
- [x] **G7.1 激光射线与 Ribbon。** 2026-09-10 恢复原 `CBullet::Bind` 射程3000、native18射程设置，取消1400上限；激光碰撞不加入弹体半径，短束仍画端点；native12/13驱动原宽度、采样间隔、RGBA和尾迹消退。七把相关武器专项与76模板回归通过，见[证据](docs/weapon-effects-fix-2026-09-10.md)。基础弹速与其余特效未穷尽，不勾选整个G7。
...
- [x] Invite/Free Warbucks 还原原弹窗、STR、图标及开闭；普通商店不足币接原三按钮，筛选按 MDS/STORE.type。
- [x] 横扫使用原 WIPE Movie、两个裁剪回调；加载 CG/文字按 BIG KEYSET/SPLASH/STR，接入进出战斗。
- [x] 删除正式路径中旧装备、星图、选轮、账户/排行榜、社交、炼化/结算等替代布局，帮助统一原14条列表。
- [x] Release、菜单/加载/推广/商店/Bank、HUD/双枪、窗口复用、四星球与Horde原存档实战、旧研究存档续玩及原结算回归均退出0。详细审计、数据来源和平台边界见[UI清理记录](docs/ui-restoration-2026-09-09.md)，不代替 UI1–UI7 整体验收。
