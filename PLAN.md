...
- `CMatrix4d` 补 `Translate(x,y,z)` / `Rotate(角度, 轴)` / 四元数转矩阵——现在只有单轴旋转和平移。
- 部件表 + 矩阵栈。**"层级"是个误称**：`DrawHeirarchy` 收的是一张扁平数组，每条 68 字节，
  内容是 `{动画控制器, 贴图, 挂点变换 pos+quat, 额外旋转 角度+轴, 缩放+颜色}`，
  骨骼号存在部件记录里，`-1` 表示不挂、留在原点（见调用方 :67642）。
- 玩家是两个 mesh 拼的：`pack0_core` mesh 0 是上半身（骨骼 `brother/head/gun/shoulder_left/back`
  正是挂点名），mesh 1 是裤子+靴子，各带各的图集。枪是第三个部件。

暂不做 `DrawHeirarchy` 里那个可选的第二遍渲染（换混合模式再画一次，疑似阴影/描边）。
