...
int RunPowerupCheck(const std::string &bigDirectory) {
    ...
    std::ofstream report("out/powerup-check.txt");
    unsigned failures = CheckTargetingController(), references = 0;
    for (const PowerupEntry &entry : catalog) {
        ...
        for (const ScriptResourceRef &resource : entry.data.script.GetResources()) {
            std::vector<std::uint8_t> payload;
            ++references;
            bool present = false;
            if (resource.sectionOrType == 253) {
                ...
                present = tables.ReadSectionResource(resource.packHash, static_cast<GameSection>(resource.sectionOrType + 1), resource.resourceId, payload);
            }
            if (!present) {
                ++failures;
        ...
        for (int tick = 0; tick < 600; ++tick) {
            ...
            for (const PowerupAction &action : actions) {
                ...
                if (action.function == 24) {
                    std::vector<std::uint8_t> payload;
                    if (!tables.ReadSectionResource(action.resource.packHash, GameSection::Bullet, action.resource.localIndex, payload)) { ++failures; }
                    else {
                        CArrayInputStream stream(payload);
                        ...
                        for (int time = 0; time < 5000 && !projectile.removed; time += 16) {
                            ...
                            for (const GunCue &cue : projectile.TakeCues()) {
                                ...
                                } else if (cue.kind == GunCue::Kind::SpawnEnemy) {
    ...
    for (std::uint8_t function : {std::uint8_t(0), std::uint8_t(23)}) {
        for (std::uint8_t count : {std::uint8_t(2), std::uint8_t(4)}) {
            ...
            if (cues.size() != 1 || cues[0].kind != GunCue::Kind::Splash ||
