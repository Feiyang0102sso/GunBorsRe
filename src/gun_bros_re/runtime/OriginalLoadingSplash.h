 ...
 */
#ifndef GUN_BROS_RE_ORIGINALLOADINGSPLASH_H
#define GUN_BROS_RE_ORIGINALLOADINGSPLASH_H
#include "runtime/OriginalTextLayout.h"
#include "runtime/OriginalSplashData.inc"
#include "engine/CKeysetResource.h"
...
public:
    bool Init(MovieRenderer &movies, unsigned index, const CProfileManager *profile = nullptr) {
        ...
        std::printf("[loading-splash] pack=%s index=%u image=%08x text=%08x movie=%u chapters=%zu\n",
            core.GetShortName().c_str(), index, m_imageHandle, m_textHandle, m_ordinal, movie->chapters.size());
...
private:
    class Header : public IMovieRegionCallback {
    public:
        Header(MovieRenderer &renderer, const CProfileManager *profile) : movies(renderer), profile(profile) {}
    ...
    bool DrawMovieRegion(const MovieRegion &region) override {
        if (region.index == 0) {
            ...
            const auto lines = FormatStoreText(*m_movies, m_text, region.width, {0, 0, 0, 0, 0});
            float y = region.y;
