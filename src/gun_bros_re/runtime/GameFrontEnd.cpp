...
public:
    ...
    int LegacyHeader(const CProfileManager &profile, const CPlayerProgress &progress, unsigned currentPage) {
        movies.Draw(10, 1600);
    ...
    bool DrawEquippedPlayer(CResTOCManager &toc, PackTables &tables, const CProfileManager &profile,
        ...
        // Legacy equipment-study framing; the store uses the original chain above.
        ...
        float centre[16], normalise[16], tilt[16], facing[16], local[16], turned[16], oriented[16], viewProjection[16];
...
unsigned StoreFilterRows(unsigned category, const char *&table) {
    if (category == 3) { table = "MDS_BUTTON_STORE_SORT_CURRENCY"; return 4; }
...
void ShowStoreFundsPrompt(MenuState &state, const std::vector<StoreEntry> &store,
    const CProfileManager &profile, unsigned currency, unsigned price) {
...
bool DrawStore(GameMenu &view, CResTOCManager &toc, PackTables &tables, CProfileManager &profile,
    ...
    for (const std::pair<int, unsigned> &row : ordered) {
        ...
        if (state.shopCategory == 0) {
            ...
            category = store[index].data.type - 14;
        } else {
            slot = 5;
            matches = MatchesEquipmentSlot(store[index], slot, weapons, armors);
        }
    ...
    if (!RequireRegion(view, storeScroll, kFirstColumnRegion, kScrollRestTime, firstSlot, "first column") ||
        !RequireRegion(view, storeScroll, kFirstColumnRegion + 1, kScrollRestTime, secondSlot, "second column") ||
    ...
    if (view.movies.Region(sortBar, kSortPanelRegion, sortTime, sortPanel)) {
        ...
        for (unsigned row = 0; row < rows; ++row) {
            ...
            if (state.shopCategory == 3) {
                bit = 0;
                if (entry->parameter >= 14 && entry->parameter <= 16) { bit = 1u << (entry->parameter - 14); }
            }
...
void DrawStarMap(GameMenu &view, MenuState &state) {
    view.movies.Draw(47, 1600); // Explicit legacy .dat research preview.
...
int GameMenu::Header(const CProfileManager &profile, const CPlayerProgress &progress, unsigned currentPage) {
    if (!profile.nativeArchive) { return LegacyHeader(profile, progress, currentPage); }
...
bool DrawLegacyRefinery(GameMenu &view, MenuState &state, CProfileManager &profile,
...
bool DrawRefinery(GameMenu &view, MenuState &state, CProfileManager &profile,
    const CRefinementManager::Template &data, const std::filesystem::path &savePath, std::int64_t now) {
    if (!profile.nativeArchive) { return DrawLegacyRefinery(view, state, profile, data, savePath, now); }
 ...
 * The historical preview below is reachable only with an explicit legacy .dat. */
...
std::string OptionsText(GameMenu &view, const CProfileManager &profile, unsigned index, unsigned slot) {
...
bool DrawOptions(GameMenu &view, MenuState &state, CProfileManager &profile, bool &saveChanged) {
    ...
    unsigned count = 0;
    while (OriginalMenuData("MDS_OPTIONS", count) != nullptr) { ++count; }
    ...
    const float wheel = view.window.TakeWheelDelta();
    if (state.optionsOpening == start && view.MouseIn(input.x, input.y, input.width, input.height)) {
        if (view.dragY != 0) {
    ...
    for (const auto &region : view.movies.Regions(list, time)) {
        ...
        for (const auto &part : view.movies.Regions(button, buttonTime, x, y)) {
            if (part.index == 1) {
                view.movies.Text(OptionsText(view, profile, index, 0), part.x,
    ...
    if (selected >= 0) {
        ...
        const auto *entry = OriginalMenuData("MDS_OPTIONS", selected);
        switch (entry->action) {
        ...
        case 1: state.Navigate(8); state.detail = 0; break;
        case 79: state.Navigate(29); break;
        case 20: state.Navigate(9); break;
        }
    }
    ...
    const auto lines = FormatStoreText(view.movies, OptionsText(view, profile, state.optionsFocus, 1),
    ...
    if (view.MouseIn(content.x, content.y, content.width, content.height)) {
...
int ShowGameMenu(CResTOCManager &toc, PackTables &tables, CProfileManager &profile,
    ...
    while (view.window.PumpEvents()) {
        ...
        for (KeyCode key = view.window.TakeKeyPress(); key != KeyCode::None; key = view.window.TakeKeyPress()) {
            ...
            if (key == KeyCode::E) {
                if (profile.nativeArchive) { state.Navigate(2); }
                else { state.Navigate(1); }
            }
        ...
        if (state.page == 27 || state.page == 28) {
            ...
            } else if (!DrawPostGame(view, state, toc, tables)) { return -3; }
        }
        ...
        if (state.page == 24 && !profile.nativeArchive) {
            auto day = LocalCalendarDay();
            if (profile.nativeArchive) { day = CurrentSeconds(); }
        ...
        if (state.page == 0 || state.page == 22) {
            if (profile.nativeArchive) {
                if (!DrawOriginalStarMap(view, state, profile)) { return -3; }
                if (!DrawOriginalModeOverlay(view, state)) { return -3; }
            } else { DrawStarMap(view, state); }
            if (!profile.nativeArchive && (!state.modeSelected || state.page == 22)) {
        ...
        if (state.page == 21) {
            ...
            } else { DrawRevolutions(view, state, profile); }
        }
        ...
        if ((state.page >= 4 && state.page <= 16) || state.page == 25 || state.page == 29) {
            view.BodyPanel();
            bool saveChanged = false;
            if (profile.nativeArchive && (state.page == 25 || state.page == 29)) {
            ...
            } else if (profile.nativeArchive && (state.page == 4 || state.page == 5 || state.page == 11 || state.page == 13)) {
                ...
                // CMenuAction::DoAction 0x4E :94882 clears firstLaunch here.
                if (view.Hit(108, 306, 388, 448)) { profile.playerBrother = 0; profile.firstLaunch = false; if (!profile.SaveToDisk(savePath)) { return -3; } return 5; }
                if (view.Hit(529, 306, 388, 448)) { profile.playerBrother = 1; profile.firstLaunch = false; if (!profile.SaveToDisk(savePath)) { return -3; } return 5; }
        ...
        if (navigation >= 7) {
            ...
            state.Navigate(17);
            state.shopFilter = 1u << state.currencyTab;
        } else if (navigation >= 0) {
            if (profile.nativeArchive && navigation == 6) {
        ...
        if (state.page != 0 && state.page != 2 && state.page != 4 && state.page != 5 && state.page != 6 && state.page != 14 && state.page != 17 && state.page != 18 && state.page != 19 && state.page != 21 && state.page != 22 &&
            state.page != 23 && state.page != 24 && !(profile.nativeArchive && (state.page == 3 || state.page == 29)) && (state.page < 25 || state.page == 29) && !state.refinementRequired && !state.currencyPending && !state.inviteOpen && view.Button(20, 708, 145, 42, "BACK")) { state.Back(); }
    ...
    for (unsigned page : {4u, 5u, 11u, 13u}) {
        for (bool credentials : {false, true}) {
            ...
            if (!DrawOriginalSocialOffline(view, state, credentials) || view.ExchangeClick(false) ||
                state.page != page || state.inviteOpen || state.socialTime < start || state.socialTime > end) { return 1; }
...
int RunOptionsCheck(const std::string &bigDirectory) {
    ...
    for (unsigned index : order) {
        ...
        view.Begin(6);
        view.SetTestClick(click);
        if (!DrawOptions(view, state, profile, changed) || state.optionsFocus != index) { return 1; }
        ...
        if (entry->action == 20 && state.page != 9) { return 1; }
...
int CheckBank(CResTOCManager &toc, PackTables &tables, const CPlayerProgress::Template &progress,
    ...
    std::sort(currencies.begin(), currencies.end());
    {
        ...
        StoreCardFace face;
        if (!probe.movies.Region(probe.movies.Ordinal("GLU_MOVIE_STORE_SCROLL"), kFirstColumnRegion, kScrollRestTime, slot)) { return 1; }
        MovieRegion secondColumn, promptTouch;
        if (!probe.movies.Region(probe.movies.Ordinal("GLU_MOVIE_STORE_SCROLL"), kFirstColumnRegion + 1, kScrollRestTime, secondColumn) ||
    ...
    for (unsigned phase = 0; phase < 9 + currencies.size(); ++phase) {
        ...
        state.shopFilter = 1;
        if (phase == 2) { state.shopFilter = 2; }
        if (phase >= 3) { state.shopFilter = 4; }
        ...
        if (phase >= 9) {
            ...
            state.shopFilter = 1u << (fixture[first].data.type - 14);
        }
        for (unsigned index = 0; index < fixture.size(); ++index) {
            const auto &item = fixture[index].data;
            if (item.type < 14 || item.type > 16 || item.displayOrder < 0 || item.value242 == 1 ||
                (state.shopFilter & (1u << (item.type - 14))) == 0) { continue; }
            if (phase >= 9) { continue; }
        ...
        for (const auto &row : currencies) {
            ...
            if ((state.shopFilter & (1u << (fixture[row.second].data.type - 14))) != 0) { ++position; }
        }
...
int RunGameMenuCheck(const std::string &bigDirectory) {
    ...
    unsigned popupOpening = 0, popupClosing = 0, starsDuration = 0;
    MenuTestClick closePopup{}, buyUpgrade{};
    {
        GameMenu probe;
        if (!probe.Open(toc, tables)) { return 1; }
        const unsigned popupOrdinal = probe.movies.Ordinal("GLU_MOVIE_UPGRADE_POPUP");
        const CMovie *popup = probe.movies.GetMovie(popupOrdinal);
        const CMovie *stars = probe.movies.GetMovie(probe.movies.Ordinal("GLU_MOVIE_WEAPON_UPGRADE_MASTERY"));
        unsigned start = 0, end = 0;
        if (popup == nullptr || stars == nullptr || !popup->GetChapterRange(0, start, end)) { return 1; }
        popupOpening = end - start + 1;
        if (!popup->GetChapterRange(2, start, end)) { return 1; }
        popupClosing = end - start + 1;
        if (!popup->GetChapterRange(1, start, end)) { return 1; }
        starsDuration = stars->duration;
        for (unsigned action : {0u, 2u}) {
            unsigned regionIndex = kUpgradeCloseRegion;
            if (action == 2) { regionIndex = kUpgradeBuyRegion; }
            MovieRegion area;
            const OriginalMenuEntry *entry = OriginalMenuData("MDS_BUTTON_STORE_UPGRADE", action);
            if (entry == nullptr || !probe.movies.Region(popupOrdinal, regionIndex, start, area)) { return 1; }
            bool found = false;
            for (const MovieRegion &touch : probe.movies.Regions(probe.movies.Ordinal(entry->movies[0]), 0, area.x, area.y)) {
                if (touch.index != 0) { continue; }
                MenuTestClick click{touch.x + touch.width / 2, touch.y + touch.height / 2};
                if (action == 0) { closePopup = click; }
                else { buyUpgrade = click; }
                found = true;
            }
            if (!found) { return 1; }
        }
    }
    // Scripted sessions have their own clock. Include bind, authored opening,
    // click and authored closing as separate frames, just like a human session.
    const std::vector<MenuTestClick> overviewClicks = {{-100, -100}, {-100, -100, popupOpening},
        closePopup, {-100, -100, popupClosing}};
    if (ShowGameMenu(toc, tables, profile, progress, refinement, store, weapons, armor,
    ...
    const std::vector<MenuTestClick> upgradeClicks = {{-100, -100}, {-100, -100, popupOpening},
        buyUpgrade, {-100, -100, starsDuration}, {-100, -100, 250}};
    if (ShowGameMenu(toc, tables, profile, progress, refinement, store, weapons, armor,
    ...
    MenuState bankState;
    bankState.page = 17;
    bankState.shopFilter = 1;
    MenuTestClick bankBuy;
    {
        GameMenu probe;
        if (!probe.Open(toc, tables)) { return 1; }
        MovieRegion slot, price, button;
        StoreCardFace face;
        const auto *entry = OriginalMenuData("MDS_BUTTON_STORE_ITEMS", kBuyButtonEntry);
        if (entry == nullptr || !probe.movies.Region(probe.movies.Ordinal("GLU_MOVIE_STORE_SCROLL"),
            kFirstColumnRegion, kScrollRestTime, slot)) { return 1; }
        face.x = slot.x;
        face.y = slot.y;
        if (!CardRegion(probe, probe.movies.Ordinal("GLU_MOVIE_SHOP_BOX"), kCardPriceRegion, face, price) ||
            !probe.movies.Region(probe.movies.Ordinal(entry->movies[0]), 1, 0, button)) { return 1; }
        bankBuy = {price.x + price.width - button.width / 2, price.y + price.height - button.height / 2};
    }
    const std::vector<MenuTestClick> bankClicks = {bankBuy, {bankBuy.x, bankBuy.y, 2000},
        {-100, -100, 2000}, {-100, -100, 1000}};
