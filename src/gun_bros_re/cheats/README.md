...
| `stboss` | 通过原 Flow 回调逐帧快进至 Boss，保持窗口响应 | 生存战斗；暂停、救援或任一方道具动画期间不可用 |
| `stsuicide` | 进入角色死亡流程 | 战斗 |
| `brow` | 请求测试 bot 正常换枪动画 | 单人选用测试好友 / Live |
| `brok` | 杀死测试 bot | 单人选用测试好友 / Live |
| `bror` | 通过原角色 Flow 复活测试 bot | 单人选用测试好友 / Live |
| `bros` | 测试 bot 打开限时 10 秒的战斗商店，不占每波自动开店次数 | Live；救援期间不可用 |
| `brop` | 测试 bot 随机使用一个可用道具；测试时可补一份库存 | Live |
