...
bool CChallengeManager::InitProgressData(CResTOCManager &toc, CGunBros &tables, CProfileManager &profile, unsigned seconds) {
...
void CChallengeManager::UpdateFromLevelSession(const Session &session, const std::vector<CGun::Entry> &weapons,
...
bool CChallengeManager::AwardAvailableRewards(CProfileManager &profile, const std::vector<CStoreItem::Entry> &store, unsigned &awarded) {
    awarded = 0;
    for (auto &challenge : current) {
        ...
        for (unsigned tier = status; tier < 3; ++tier) {
            ...
            for (const auto &ref : prize.storeItems) {
                const CStoreItem::Entry *item = nullptr;
                ...
                if (result != CProfileManager::PurchaseResult::Purchased && result != CProfileManager::PurchaseResult::Owned && result != CProfileManager::PurchaseResult::LevelLocked) { return false; }
            }
