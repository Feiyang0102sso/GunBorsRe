...
#include <cstdio>
bool CPlayerProgress::Template::Load(CResTOCManager &toc, CGunBros &tables, CPlayerProgress::Template &data) {
    for (unsigned packIndex = 0; packIndex < toc.GetPackCount(); ++packIndex) {
        ...
        if (!tables.ReadSectionResource(toc.GetPack(packIndex)->GetPackHash(), ZGameSection::PlayerProgress, 0, payload)) { continue; }
