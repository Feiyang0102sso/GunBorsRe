...
public:
    class Template {
    public:
        static bool Load(CResTOCManager &toc, CGunBros &tables, Template &data);
    ...
    /** Native client 1000; keep unconsumed bytes in the manager's record. */
    static void ReadProfileData(CProfileManager &candidate, const std::vector<std::uint8_t> &bytes);
    static void WriteProfileData(const CProfileManager &profile, std::vector<std::uint8_t> &progress,
        const Template &progression, std::uint64_t loadedExperience);
