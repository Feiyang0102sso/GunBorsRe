...
#include "gun_bros_re/data/profile/CPlayerConfiguration.h"
...
#include "gun_bros_re/data/profile/COptionsMgr.h"
#include <optional>
#include <filesystem>
#include <string>
#include <bitset>
...
public:
    ...
    /** Native DataStore protocol, original CProfileManager 202880/203163. */
    static bool ReadEnvelope(const std::filesystem::path &path, Envelope &record);
    static bool ReadRecord(const std::filesystem::path &path, unsigned id, Record &record);
    static bool WriteArchive(const std::filesystem::path &directory, const Archive &archive);
    static bool CreateArchive(CResTOCManager &toc, CGunBros &tables, Archive &archive);
    bool CreateTransient(CResTOCManager &toc, CGunBros &tables);
    bool LoadNative(CResTOCManager &toc, CGunBros &tables, const std::filesystem::path &directory,
        const std::filesystem::path &sourceDirectory = {});
    bool ImportNative(CResTOCManager &toc, CGunBros &tables, const std::filesystem::path &source);
    bool ReloadNative(const std::filesystem::path &directory);
    bool SaveNative(const std::filesystem::path &directory) const;
    bool RecordMissionWaves(const GameObjectRef &level, unsigned waveProgress,
        const std::vector<bool> &perfectResults);
    bool RecordMissionScore(const GameObjectRef &mission, unsigned score);
    ...
    std::optional<Archive> nativeArchive;
