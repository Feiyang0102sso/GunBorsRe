...
public:
    ...
    bool Bind(CResTOCManager &toc, CGunBros &tables, const CProfileManager &profile, unsigned seconds);
    /** Apply the original forward-only network day rollover, then bind its list. */
    bool InitProgressData(CResTOCManager &toc, CGunBros &tables, CProfileManager &profile, unsigned seconds);
    void UpdateFromLevelSession(const Session &session, const std::vector<CGun::Entry> &weapons, const CProfileManager &profile);
    ...
    bool AwardAvailableRewards(CProfileManager &profile, const std::vector<CStoreItem::Entry> &store, unsigned &awarded);
    unsigned cycleDay = 0;
