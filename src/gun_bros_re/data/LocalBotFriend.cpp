...
bool LocalBotFriend::Load(CResTOCManager &toc, PackTables &tables, const std::filesystem::path &playerPath, const CProfileManager *seed) {
    ...
    if (!LoadNativeProfile(toc, tables, profile, m_directory)) { return false; }
...
bool LocalBotFriend::Save() const {
...
bool LocalBotFriend::Select(bool active) {
