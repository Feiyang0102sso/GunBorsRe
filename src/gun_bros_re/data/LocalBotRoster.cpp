...
bool Reference(CResTOCManager &toc, PackTables &tables, const std::string &value, GameSection section, GameObjectRef &ref) {
    if (value == "none" && section == GameSection::Armor) { ref = {}; return true; }
...
std::string ReferenceText(PackTables &tables, const GameObjectRef &ref) {
    if (ref.IsNull()) { return "none"; }
    return tables.GetPackName(ref.packHash) + ":" + std::to_string(ref.localIndex);
}
struct Definition { std::string identity; std::map<std::string, std::string> values; };
}

LocalBotFriend *LocalBotRoster::At(unsigned index) const {
    if (index >= m_friends.size()) { return nullptr; }
    return m_friends[index].get();
}
bool LocalBotRoster::Select(unsigned index) {
...
LocalBotFriend *LocalBotRoster::MatchSelected() const {
...
bool LocalBotRoster::Load(CResTOCManager &toc, PackTables &tables, const std::filesystem::path &playerPath, CProfileManager &player) {
    ...
    for (const auto &definition : definitions) {
        auto entry = std::make_unique<LocalBotFriend>();
        ...
        for (const auto &[key, value] : definition.values) {
            ...
            if (key == "level") {
                ...
                if (!Reference(toc, tables, value, GameSection::Gun, ref)) { return false; }
                ...
                if (!Reference(toc, tables, value, GameSection::Armor, ref)) { return false; }
                if (applyChanges) {
                ...
                if (!Number(value, number) || !Reference(toc, tables, key.substr(8), GameSection::Powerup, ref)) { return false; }
