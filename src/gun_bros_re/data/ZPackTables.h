...
#ifndef GUN_BROS_RE_MILESTONES_ZPACKTABLES_H
#define GUN_BROS_RE_MILESTONES_ZPACKTABLES_H
...
#include "engine/resources/ZResourcePacks.h"

class ZPackTables {
public:
    explicit ZPackTables(CResTOCManager &tocManager) : m_tocManager(tocManager), m_resources(tocManager) {
    ...
    bool HasLatestBigVersion() const {
        ...
        for (const CGameObjectPack &pack : m_objectPacks) {
            if (pack.GetBigVersion() != ZBigVersion::V1) { return false; }
        }
        return true;
    }
    void SetLoadProgress(ZPackLoadProgress *progress) { m_resources.SetLoadProgress(progress); }
    ...
    bool ReadSectionResource(std::uint32_t packHash, ZGameSection section,
