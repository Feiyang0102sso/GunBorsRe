...
public:
    ...
    CGameObjectPack();
    ~CGameObjectPack();
    CGameObjectPack(CGameObjectPack &&) noexcept;
    CGameObjectPack &operator=(CGameObjectPack &&) noexcept;
    CGameObjectPack(const CGameObjectPack &) = delete;
    CGameObjectPack &operator=(const CGameObjectPack &) = delete;
    /** Lazily initialized templates remain stable until this pack is reset. */
    Objects &GetObjects();
...
private:
    std::unique_ptr<Objects> m_objects;
