...
bool CombatScene::FindMatchRoute(float x, float y, float goalX, float goalY, std::vector<CollisionPoint> &route) const {
    ...
    const auto point = [&](int index) { return CollisionPoint(m_left + index % columns * step, m_top + index / columns * step); };
    ...
    const auto clearEdge = [&](int current, const CollisionPoint &origin, const CollisionPoint &target) {
