...
void CProp::Bind(const Template &data, const std::vector<std::vector<std::uint16_t>> *durations) {
    ...
    // CProp::Bind :124916 randomizes the main frame before running Flow export 0.
    const int mainAnimation = GetAnimation(1);
    if (m_durations && mainAnimation >= 0 && mainAnimation < static_cast<int>(m_durations->size())) {
        const auto count = (*m_durations)[mainAnimation].size();
        if (count > 0) { m_players[1].SetStep(RandomInteger(0, static_cast<std::int16_t>(count - 1))); }
    }
...
void CProp::Update(int deltaMs, bool playerInside) {
    ...
    hitFlashRemainingMs = std::max(0.0f, hitFlashRemainingMs - deltaMs);
    if (m_timerMs > 0) {
        m_timerMs -= deltaMs;
    ...
    if (m_checkEntry && playerInside != m_inside) {
        ...
        if (playerInside) {
            ...
            CProp::Action action;
            action.kind = CProp::Action::Kind::Entered;
    ...
    if (m_move >= 0) {
        ...
        if (step != m_lastMoveStep && step < static_cast<int>(move.frames.size())) {
            ...
            if (sound != 255) {
                CProp::Action action;
                action.kind = CProp::Action::Kind::Sound;
...
void CProp::QueueResource(CProp::Action::Kind kind, int resource, int group) {
    CProp::Action action;
    action.kind = kind;
    action.group = group;
...
std::int16_t CProp::FunctionResolver(std::uint8_t function, const std::int16_t *arguments, std::uint8_t count) {
    ...
    case 2:
        if (count < 2) { second = 3; }
        QueueResource(CProp::Action::Kind::Effect, first, second);
        break;
    case 3:
        if (m_health > 0) {
            m_health -= m_damage;
        ...
        break;
    case 4: QueueResource(CProp::Action::Kind::Sound, first); break;
    case 5:
    case 6:
    ...
    case 10: {
        CProp::Action action;
        action.kind = CProp::Action::Kind::Splash;
        action.radius = first;
    ...
    case 9: {
        CProp::Action action;
        action.kind = CProp::Action::Kind::Destroyed;
        m_actions.push_back(action);
        m_collisionChanged = true;
        break;
    }
    ...
    case 15: QueueResource(CProp::Action::Kind::Portal, first); break;
    case 16: QueueResource(CProp::Action::Kind::AttachedEffect, first, second); break;
    case 17: {
        CProp::Action action;
        action.kind = CProp::Action::Kind::StopEffect;
        m_actions.push_back(action);
        break;
    }
...
std::vector<CProp::Action> CProp::TakeActions() {
    std::vector<CProp::Action> actions;
