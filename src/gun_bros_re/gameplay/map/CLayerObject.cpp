 ...
 */
void ReadObjectExtra(CArrayInputStream &stream, CLayerObject::Object &object) {
    const std::uint8_t objectType = object.objectType;
    if (objectType == static_cast<std::uint8_t>(CLayerObject::ObjectType::Player)) {
    ...
    } else if (objectType == static_cast<std::uint8_t>(CLayerObject::ObjectType::Enemy)) {
        object.pathLayer = stream.ReadUInt8();
        object.facing = stream.ReadInt16();
    } else if (objectType == static_cast<std::uint8_t>(CLayerObject::ObjectType::Platform)) {
