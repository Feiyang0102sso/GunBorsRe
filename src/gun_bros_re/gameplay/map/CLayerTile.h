...
public:
    ...
    // Original CLayerTile::DrawBackground; atlas submission is a desktop adapter.
    std::uint32_t DrawBackground(ZQuadBatch &batch, const TileSet &tileSet,
     ...
     */
    const Cell &GetCell(std::uint32_t column, std::uint32_t row) const;
