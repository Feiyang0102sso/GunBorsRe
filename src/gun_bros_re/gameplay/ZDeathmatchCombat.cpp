...
bool ZCombatWorld::IsMatchSpawnPending(unsigned peer) const {
...
void ZCombatWorld::SetDeathmatch(CMPMatch *match, const std::vector<ZWeaponEntry> *weapons, ZPickupScene *pickups) {
    m_match = match; m_matchWeapons = weapons; m_matchPickups = pickups;
}
GameObjectRef ZCombatWorld::ActiveMatchGun(unsigned peer) const {
    if (peer == 0) { return m_player.gunResource; }
    return m_brotherModel->gunResource;
}
bool ZCombatWorld::SelectMatchGun(unsigned peer, unsigned slot, const GameObjectRef &gun) {
...
bool ZCombatWorld::EquipMatchGun(unsigned peer, const GameObjectRef &ref, bool resetActor) {
    ZPlayerModel *model = &m_player;
...
bool ZCombatWorld::StartDeathmatch() {
...
bool ZCombatWorld::RespawnDeathmatch(unsigned peer, bool initial, bool resumeFromShop) {
...
void ZCombatWorld::RecordMatchDeath(unsigned peer, int killer) {
...
bool ZCombatWorld::AdvanceDeathmatchEnding(int deltaMs) {
    ...
    ZPlayerModel *models[] = {&m_player, m_brotherModel};
...
void ZCombatWorld::UpdateDeathmatch(unsigned deltaMs) {
...
bool ZCombatWorld::CollectMatchWeapon(unsigned peer, unsigned index) {
...
bool ZCombatWorld::RequestMatchWeaponSwap(unsigned peer) {
...
bool ZCombatWorld::FinishMatchWeaponSwap(unsigned peer) {
    m_matchSwap[peer] = false;
    m_auxiliaryMs[peer] = 0;
    m_matchSlots[peer] = 1 - m_matchSlots[peer];
    return EquipMatchGun(peer, m_gunConfigurations[peer][m_matchSlots[peer]]);
}
bool ZCombatWorld::CanHitBrother(const ZCombatHit &hit, ZCombatId target) const {
    if (m_match != nullptr) {
...
ZCombatId ZCombatWorld::ParticipantOwner(ZCombatId owner) const {
...
bool ZCombatWorld::HasLineOfFire(float x, float y, float targetX, float targetY) const {
...
bool ZCombatWorld::FindMatchDestination(float x, float y, bool cover, float targetX, float targetY, float &goalX, float &goalY, unsigned choice) const {
...
bool ZCombatWorld::FindMatchSupply(float x, float y, float &goalX, float &goalY) const {
