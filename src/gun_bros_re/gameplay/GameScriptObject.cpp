...
std::int16_t *ResolveVariable(GameScriptObject *host, std::uint16_t variableId) {
    ...
    if (classId == kScriptClassGame) {
        if (variable == 4 && host->GetLevelContext() != nullptr) {
            return host->GetLevelContext()->ResolveGameVariable(variable);
        }
        if (variable == 6) {
            CLevel *level = dynamic_cast<CLevel *>(host);
            if (level == nullptr) { level = host->GetLevelContext(); }
...
std::int16_t GameScriptObject::ResolveNativeFunction(std::uint16_t id, const std::int16_t *arguments, std::uint8_t count) {
    return GameScriptResolver::ResolveFunction(this, id, arguments, count);
}
std::int16_t *GameScriptObject::ResolveNativeVariable(std::uint16_t id) {
    return GameScriptResolver::ResolveVariable(this, id);
}
