...
bool PowerupScene::Init() {
    ...
    for (const auto &entry : m_catalog) {
        ...
        for (const auto &ref : entry.data.script.GetResources()) {
            ...
            if (ref.sectionOrType != static_cast<unsigned>(GameSection::Bullet) - 1) { continue; }
            std::vector<std::uint8_t> bytes;
            if (!m_tables.ReadSectionResource(ref.packHash, GameSection::Bullet, ref.resourceId, bytes)) { return false; }
...
const CStoreItem *PowerupScene::FindStoreItem(const PowerupEntry &entry) const {
...
bool PowerupScene::ModeAllows(const PowerupEntry &entry) const {
...
bool PowerupScene::HasUnlimitedMatchInventory() const {
...
bool PowerupScene::CanBotUseSelected() const {
