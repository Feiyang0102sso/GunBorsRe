...
ZEnemyTemplateData::ZEnemyTemplateData()
    : packHash(0),
      ordinal(0),
      gameScale(0.0f),
      uiScalePercent(0.0f),
...
bool ReadEnemyTemplate(ZPackTables &tables, std::uint32_t packHash,
                       std::uint32_t ordinal, const std::string &owner,
                       ZEnemyTemplateData &out) {
...
bool LoadEnemyCatalog(CResTOCManager &toc, ZPackTables &tables,
    std::vector<ZEnemyTemplateData> &entries) {
    entries.clear();
    bool complete = true;
...
static bool LoadEnemyConfigs(ZPackTables &tables, const ZEnemyTemplateData &entry,
    bool createBuffers, const ZShaderProgram *program,
    std::vector<std::shared_ptr<ZEnemyModelConfig>> &configs, ZEnemyModelCache *cache) {
    configs.clear();
    ...
    for (std::size_t i = 0; !cached && i < entry.moveSet.GetMeshConfigs().size(); ++i) {
        ...
        auto loaded = std::make_shared<ZEnemyModelConfig>();
...
bool PreloadEnemyModel(ZPackTables &tables, const ZEnemyTemplateData &entry,
    const ZShaderProgram &program, ZEnemyModelCache &cache) {
    std::vector<std::shared_ptr<ZEnemyModelConfig>> configs;
...
bool LoadEnemyModel(ZPackTables &tables, const ZEnemyTemplateData &entry,
                    bool createBuffers, const ZShaderProgram *program,
                    ZEnemySpawnMode spawnMode, ZEnemyModel &out, ZEnemyModelCache *cache) {
    ...
    if (spawnMode == ZEnemySpawnMode::Level) {
        out.enemy.Spawn();
    } else {
...
std::int32_t EnemyPartConfig(const ZEnemyModel &model, std::uint32_t partIndex) {
...
void DrawEnemyModel(ZEnemyModel &model, const ZShaderProgram &program,
                    const float *base) {
    ...
    for (std::uint32_t i = 0; i < model.enemy.GetPartCount(); ++i) {
        ...
        ZEnemyModelConfig &config = *model.configs[configIndex];
...
float EnemyModelWorldScale(const ZEnemyModel &model, float gameScale,
                           float cameraScale) {
...
void BuildEnemyGameMatrix(const ZEnemyModel &model, const float *base, float x,
    ...
    const std::int32_t configIndex = EnemyPartConfig(model, 0);
    if (configIndex >= 0) {
        const ZMeshBounds &bounds = model.configs[configIndex]->mesh.GetBounds();
