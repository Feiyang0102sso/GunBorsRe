...
void ZCombatWorld::SetPlayerProgress(CPlayerProgress *progress) {
    m_actor.BindProgress(progress);
}

void ZCombatWorld::AddExperience(unsigned amount) {
...
void ZCombatWorld::RewardEnemy(const ZCombatEnemy &actor) {
    if (m_actor.GetProgress() == nullptr || m_level == nullptr) { return; }
...
void ZCombatWorld::UpdateExperienceTexts(int deltaMs) {
    if (deltaMs <= 0) { return; }
...
void ZCombatWorld::AddHealth(unsigned amount) {
    m_actor.AddHealth(amount);
}

void ZCombatWorld::AddXplodium(unsigned amount) {
    if (m_localLive && IsTeamDeathComplete()) { return; }
...
unsigned ZCombatWorld::GetTotalKills() const {
...
float ZCombatWorld::GetEnemyTimeScale() const {
    if (m_level == nullptr) { return 1; }
    return m_level->GetObjectTimeScale();
}

bool ZCombatWorld::TouchesPickup(float x, float y) const {
    ...
    return !IsMatchSpawnPending(0) && !m_vitals.dead && CircleFraction(m_actor.x, m_actor.y,
        m_actor.previousX - m_actor.x, m_actor.previousY - m_actor.y, x, y, m_playerRadius + 10) <= 1;
}

void ZCombatWorld::OnWaveCleared(unsigned perfectRewardPercent) {
...
ZCombatWorld::ZCombatWorld(ZPackTables &tables, const ZShaderProgram &program,
    ...
    : m_actor(player, vitals), m_tables(tables), m_program(program), m_catalog(catalog), m_player(player),
      m_vitals(vitals), m_effects(effects), m_playerGameScale(playerGameScale) {
    m_effects.SetCombatWorld(this);
}

void ZCombatWorld::SetMap(CMap &map, const CCollisionData &collision, ZWeaponCollision &weaponCollision,
...
void ZCombatWorld::ResolveMovement(float previousX, float previousY, float &x, float &y, float radius, bool player) const {
...
bool ZCombatWorld::HasClearPath(float x, float y, float targetX, float targetY, float radius) const {
...
bool ZCombatWorld::CanWalkTo(float x, float y, float targetX, float targetY) const {
...
void ZCombatWorld::ResolveBrotherForce(float previousX, float previousY, float &x, float &y) {
    ResolveMovement(previousX, previousY, x, y, m_playerRadius);
}

void ZCombatWorld::UpdateNavigation(ZCombatEnemy &actor, int deltaMs) {
    PerformanceProbe::Scope timing(PerformanceProbe::counters.navigationMs);
    ZEnemyCombat &state = actor.model.enemy.combat;
...
bool ZCombatWorld::PreloadEnemies(const RequirementList &requirements, const CScript &levelScript) {
    ...
    for (std::size_t index = 0; index < pending.size(); ++index) {
        ...
        if (ref.sectionOrType != static_cast<unsigned>(ZGameSection::Enemy) - 1 || ref.resourceId == 255) { continue; }
...
ZCombatEnemy *ZCombatWorld::Spawn(std::size_t entry, float x, float y) {
    ...
    if (!LoadEnemyModel(m_tables, *actor->data, true, &m_program, ZEnemySpawnMode::Level, actor->model, &m_enemyModelCache)) {
        ++invalidSpawns;
        return nullptr;
    }
...
ZCombatEnemy *ZCombatWorld::SpawnNearby(std::size_t entry) {
    float radius = 40;
    ...
    for (int attempt = 0; attempt < 120; ++attempt) {
        ...
        const float x = m_actor.x + std::sin(angle) * distance;
        const float y = m_actor.y - std::cos(angle) * distance;
...
ZCombatEnemy *ZCombatWorld::Find(ZCombatId id) {
...
std::size_t ZCombatWorld::AliveCount() const {
    std::size_t count = 0;
    for (const auto &actor : enemies) {
        const ZEnemyCombat &state = actor->model.enemy.combat;
...
void ZCombatWorld::PlayerMatrix(float *matrix) const {
    float identity[16];
...
void ZCombatWorld::SetBrother(ZPlayerModel *model, CBrotherAI *brother) {
    m_brotherModel = model;
    m_brother = brother;
}

void ZCombatWorld::SetBrotherWeapons(const CScript &script, const CGun::Template &pistol, const CGun::Template &rifle) {
...
bool ZCombatWorld::RequestBrotherWeaponSwap() {
...
bool ZCombatWorld::SwapBrotherWeapon() {
...
void ZCombatWorld::ResetBrotherPosition(float x, float y, float facingDegrees) {
...
void ZCombatWorld::BrotherMatrix(float *matrix) const {
    float identity[16];
    Matrix4dIdentity(identity);
...
ZCombatId ZCombatWorld::FindBrotherTarget(float x, float y, float radius) {
    if (IsDeathmatch()) {
...
bool ZCombatWorld::GetBrotherTarget(ZCombatId id, float &x, float &y) {
...
bool ZCombatWorld::GetBrotherWaypoint(float x, float y, float targetX, float targetY,
...
void ZCombatWorld::EnemyMatrix(const ZCombatEnemy &actor, float *matrix) const {
    float identity[16];
    Matrix4dIdentity(identity);
    const ZEnemyCombat &state = actor.model.enemy.combat;
...
void ZCombatWorld::PartMatrix(const ZCombatEnemy &actor, int index, float *matrix) const {
    float base[16];
...
bool ZCombatWorld::Anchor(ZCombatId id, int part, int node, float &x, float &y, float &z, float &direction) {
...
void ZCombatWorld::SelectTarget(ZCombatEnemy &actor) {
    CEnemy &enemy = actor.model.enemy;
...
void ZCombatWorld::EnemyCircle(const ZCombatEnemy &actor, int part, float &x, float &y, float &radius) const {
    const CEnemy &enemy = actor.model.enemy;
...
ZCombatTrace ZCombatWorld::Trace(const ZCombatHit &hit, float x, float y, float dx, float dy,
    ...
    for (auto &actor : enemies) {
        CEnemy &enemy = actor->model.enemy;
        ZEnemyCombat &state = enemy.combat;
        ...
        for (std::uint32_t p = 0; p < enemy.GetPartCount(); ++p) {
            const ZEnemyPart &part = enemy.GetPart(p);
...
std::vector<ZCombatWorld::HealthBar> ZCombatWorld::EnemyHealthBars(float viewportScale) const {
...
bool ZCombatWorld::Suicide() {
    m_player.weapon->brother.SetLevelContext(m_level);
    return m_player.weapon->brother.StartDeath();
}

ZHitResult ZCombatWorld::ApplyHit(ZCombatId target, const ZCombatHit &hit) {
    ...
    if (target == kPlayerCombatId) {
        if (!CanHitBrother(hit, target) || m_player.weapon == nullptr) { return ZHitResult::Ignored; }
        m_player.weapon->brother.SetLevelContext(m_level);
        ...
        const ZHitResult result = m_player.weapon->brother.ReceiveDamage(damage);
...
float ZCombatWorld::GetDamageMultiplier(ZCombatId owner, float fallback) const {
...
float ZCombatWorld::GetProjectilePowerupMultiplier(ZCombatId owner) const {
...
bool ZCombatWorld::FindTarget(const ZCombatHit &hit, float radius, float &x, float &y) {
    if (IsDeathmatch()) {
...
void ZCombatWorld::Splash(const ZCombatHit &hit, float radius, float coneDegrees, float force, int forceMs) {
...
void ZCombatWorld::SplashBrothers(float x, float y, float radius, float damage, float force, int forceMs) {
...
void ZCombatWorld::ApplyBrotherForce(ZCombatId target, float x, float y, int durationMs) {
    if (target == kBrotherCombatId && m_brotherModel != nullptr) {
...
void ZCombatWorld::SpawnFromProjectile(const GameObjectRef &resource, const ZCombatHit &hit) {
...
void ZCombatWorld::Actions(ZCombatEnemy &actor) {
    CEnemy &enemy = actor.model.enemy;
    const ZEnemyCombat &state = enemy.combat;
    for (const ZEnemyAction &action : enemy.TakeActions()) {
        ...
        } else if (action.kind == ZEnemyAction::Kind::SpawnPickup) {
            pickupSpawns.push_back({action.resource, x, y});
        } else if (action.kind == ZEnemyAction::Kind::Bullet) {
        ...
        } else if (action.kind == ZEnemyAction::Kind::Stun) {
        ...
        } else if (action.kind == ZEnemyAction::Kind::CollisionResolved) {
        ...
        } else if (action.kind == ZEnemyAction::Kind::RemoveBullet) {
            m_effects.RemoveOldestProjectile(state.id);
        } else if (action.kind == ZEnemyAction::Kind::Broadcast) {
            for (auto &other : enemies) {
                if (other.get() != &actor && !other->model.enemy.combat.dead) {
                    other->model.enemy.TriggerEvent(static_cast<std::uint8_t>(action.slot));
                }
            }
        } else if (action.kind == ZEnemyAction::Kind::Splash || action.kind == ZEnemyAction::Kind::SpawnEnemy) {
            ZCombatHit hit;
            hit.owner = state.id;
            hit.ownerType = ownerType;
            hit.x = x; hit.y = y; hit.direction = direction;
            hit.damage = action.damage * GetDamageMultiplier(state.id);
            if (action.kind == ZEnemyAction::Kind::Splash) { Splash(hit, action.radius, 360, action.force, action.durationMs); }
            else { SpawnFromProjectile(action.resource, hit); }
        } else {
            ZGunCue cue;
            cue.resource = action.resource;
            cue.kind = ZGunCue::Kind::Effect;
            if (action.kind == ZEnemyAction::Kind::Sound) { cue.kind = ZGunCue::Kind::Sound; }
            if (action.kind == ZEnemyAction::Kind::LoopSound) { cue.kind = ZGunCue::Kind::LoopSound; }
            if (action.kind == ZEnemyAction::Kind::StopSound) { cue.kind = ZGunCue::Kind::StopSound; }
            if (action.kind == ZEnemyAction::Kind::LinkedEffect) { cue.kind = ZGunCue::Kind::Trail; }
            if (action.kind == ZEnemyAction::Kind::StopEffect) { cue.kind = ZGunCue::Kind::StopTrail; }
            if (action.kind == ZEnemyAction::Kind::Shake || action.kind == ZEnemyAction::Kind::Reward) { continue; }
            m_effects.Emit(cue, x, y, z, direction, state.id, action.slot, action.part, action.node);
        }
    }
}

void ZCombatWorld::Update(int deltaMs, float moveX, float moveY, bool shoot) {
    ...
    levelEvents.clear();
    pickupSpawns.clear();
    m_actor.BeginMovement();
    const bool moving = m_actor.UpdateMovement(deltaMs, moveX, moveY);
    m_actor.UpdateShooting(deltaMs, moving, shoot, *this);
    ...
    m_actor.ApplyKnockback(deltaMs, forceSeconds);
    if (!IsMatchSpawnPending(0)) { m_actor.Move(*this); }
    if (m_brotherModel != nullptr) {
        m_brotherModel->weapon->brother.SetLevelContext(m_level);
    ...
    for (auto &actor : enemies) {
        ...
        if (!state.dead && state.variables[16] != 1 && state.targetType != 2 && !m_vitals.dead &&
            ...
            actor->contactTimer == 0 && CombatGeometry::CircleCircle({m_actor.previousX, m_actor.previousY},
                {m_actor.x, m_actor.y}, m_player.weapon->brother.GetRadius(),
                {state.previousX, state.previousY}, {state.x, state.y}, enemy.GetPart(0).radius, contactFraction)) {
    ...
    PlayerMatrix(matrix);
    {
        ...
        m_effects.Update(m_player, matrix, m_actor.facing, deltaMs, m_weaponCollision);
    }
    for (auto &actor : enemies) {
        Actions(*actor);
        ZEnemyCombat &state = actor->model.enemy.combat;
    ...
    for (std::size_t i = 0; i < enemies.size();) {
        ZEnemyCombat &state = enemies[i]->model.enemy.combat;
...
bool ZCombatWorld::IsTeamDeathComplete() const {
...
bool ZCombatWorld::NeedsDeathChoice(unsigned peer) const {
...
void ZCombatWorld::FinishDeathChoice(unsigned peer) {
...
bool ZCombatWorld::ReviveActor(ZCombatId actor, unsigned reason) {
...
void ZCombatWorld::ActorPosition(ZCombatId actor, float &x, float &y) const {
    x = m_actor.x; y = m_actor.y;
    if (actor == kBrotherCombatId && m_brother != nullptr) { x = m_brother->x; y = m_brother->y; }
}

std::vector<ZBrotherAIWorld::Threat> ZCombatWorld::GetBrotherThreats() const {
...
void ZCombatWorld::UpdateLocalRevive(int deltaMs) {
