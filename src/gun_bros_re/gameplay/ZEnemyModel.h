...
#ifndef GUN_BROS_RE_MILESTONES_ZENEMYMODEL_H
#define GUN_BROS_RE_MILESTONES_ZENEMYMODEL_H
...
struct ZEnemyModelConfig {
    CMesh mesh;
    ZTexture texture;
    ZMeshBuffer buffer;
    bool valid;

    ZEnemyModelConfig() : valid(false) {}
};
 ...
 */
struct ZEnemyModelCache {
    std::map<std::uint64_t, std::vector<std::shared_ptr<ZEnemyModelConfig>>> entries;
...
bool LoadEnemyModel(ZPackTables &tables, const ZEnemyTemplateData &entry,
                    ...
                    ZEnemySpawnMode spawnMode, ZEnemyModel &out, ZEnemyModelCache *cache = nullptr);
