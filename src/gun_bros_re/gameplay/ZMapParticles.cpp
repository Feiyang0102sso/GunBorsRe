...
bool EnsureParticleEffectVisual(CResTOCManager &tocManager, ZLoadedMap &loaded,
    ...
    ZPackResources *spritePack = GetPackResources(tocManager, loaded,
                                                  spritePackIndex);
...
void StartTransitionParticlesForProp(CResTOCManager &tocManager,
    ...
    const ZInteractivePropKind kind = prop.sprite->interactiveKind;
    if (kind == ZInteractivePropKind::Cover && state >= 1 && state <= 3) {
        firstResource = 3;
        firstZGroup = 5;
        secondResource = 1;
        secondZGroup = 2;
    } else if (kind == ZInteractivePropKind::Barrel && state == 3) {
        firstResource = 0;
        secondResource = 1;
    } else if (kind == ZInteractivePropKind::Spire && state == 1) {
        firstResource = 0;
    } else if (kind == ZInteractivePropKind::Spire && state == 2) {
        firstResource = 1;
    }
...
void AddParticleQuads(const ZLoadedMap &loaded, ZQuadBatch &batch,
    ...
    for (std::size_t activeIndex = 0;
        ...
        const ZActiveParticleEffect &active =
        ...
        const ZParticleEffectVisual &visual = found->second;
