...
const StoreEntry *DeathmatchBot::ChoosePurchase(const std::vector<StoreEntry> &store, const CProfileManager &profile, unsigned level, const CMPMatch::Life &life) {
...
void DeathmatchBot::PrintNavigation() const {
...
std::array<unsigned, 2> DeathmatchBot::ChooseLoadout(const CMPMatch::Entry &match, const std::vector<WeaponEntry> &weapons, unsigned seed) {
...
void DeathmatchBot::Update(int deltaMs, CBrother &brother, IBrotherAIWorld &world, float, float, float speedMultiplier) {
    auto &scene = static_cast<CombatScene &>(world);
    if (deltaMs <= 0) { return; }
