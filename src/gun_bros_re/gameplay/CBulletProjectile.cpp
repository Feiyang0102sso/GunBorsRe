 ...
 */
bool ZProjectileView::PastBounds(float x, float y, float radius, float velocityX, float velocityY) const {
...
void CBullet::UpdateProjectile(ZProjectileWorld *world, ZSpriteRenderer &sprites,
    ...
    const CGameSpriteGluRef &sprite = visual->data.GetSpriteRef();
    ...
    if (world != nullptr && seekRadius > 0 && !beam) {
        float targetX = 0, targetY = 0;
        if (world->FindTarget(hit, seekRadius, targetX, targetY)) {
            direction = std::atan2(targetY - y, targetX - x) / kRadians;
        }
    }
    ...
    if (!hitWall && beam) {
        ...
        if ((flags & 0x10) == 0 &&
            view.PastBounds(x, y, visual->data.GetRadius(), dx, dy)) {
            removed = true;
            hitWall = false;
        }
    }
    ...
    if (world != nullptr && HasActiveCollision() && !removed) {
        ...
        for (int contact = 0; contact < 64; ++contact) {
            ...
            float radius = visual->data.GetRadius();
            if (beam) { radius = 0; }
            const ZCombatTrace trace = world->Trace(hit, traceX, traceY, dx, dy, radius, skip);
            ...
            hit.part = trace.part;
            hit.edge = trace.edge;
            const ZHitResult result = world->ApplyHit(trace.target, hit);
            pendingHit = result == ZHitResult::Pending;
            OnCollision(result);
