...
bool ZPowerupMoviePlayer::Start(const ZPowerupEntry &entry, bool fromSelector) {
    ...
    if (!m_particles) {
        ...
        m_particles = std::make_unique<ZWeaponEffects>(m_toc, m_tables, m_program);
    }
    Reset();
    if (fromSelector) {
...
bool ZPowerupMoviePlayer::StartMovie(const ZPowerupAction &action) {
...
bool ZPowerupMoviePlayer::ApplyActions() {
...
void ZPowerupMoviePlayer::Update(int deltaMs) {
    if (deltaMs <= 0) { return; }
