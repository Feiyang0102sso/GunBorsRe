...
const CStoreItem::Entry *ZLocalPVPBot::ChoosePurchase(const std::vector<CStoreItem::Entry> &store, const CProfileManager &profile, unsigned level, const CMPMatch::Life &life) {
    ...
    for (const auto &owned : profile.powerups) {
        if (IsHealthPowerup(owned.resource)) { healthCount += owned.count; }
    }
    ref.localIndex = 13;
...
bool ZLocalPVPBot::TakePowerupRequest() {
    if (m_level == ZBotSettings::Difficulty::Easy || vitals.dead || m_powerupDecisionMs > 0 || (!m_visible && !WantsHealth())) { return false; }
    m_powerupDecisionMs = PowerupDecisionMs;
    return true;
}

void ZLocalPVPBot::UsePowerups(CPowerUpSelector &powerups) {
    if (m_level != ZBotSettings::Difficulty::Easy) {
        if (!TakePowerupRequest()) { return; }
        if (WantsHealth() && ZLocalPVPBot::UseMatchConsumable(powerups, false)) { return; }
        if (m_level == ZBotSettings::Difficulty::Hard) { ZLocalCoopBot::UseAnyPowerup(powerups, m_powerupChoice); }
...
void ZLocalPVPBot::Update(int deltaMs, CBrother &brother, ZBrotherAIWorld &world, float, float, float speedMultiplier) {
    ...
    const bool visible = scene.GetBrotherTarget(Collision::Player, targetX, targetY) &&
    ...
    m_visible = visible;
    m_target = 0;
    if (visible) { m_target = Collision::Player; }
...
bool ZLocalPVPBot::HasUnlimitedInventory(const CPowerUpSelector &selector) {
    return selector.m_match != nullptr && selector.m_match->HasUnlimitedBotPowerups() && selector.m_owner == Collision::Brother;
}
...
bool ZLocalPVPBot::UseMatchConsumable(CPowerUpSelector &selector, bool grenade) {
    ...
    for (auto entry = selector.m_resources.m_powerups.rbegin(); entry != selector.m_resources.m_powerups.rend(); ++entry) {
        ...
        if (!grenade && !IsHealthPowerup(entry->resource)) { continue; }
        if (selector.SelectResource(entry->resource) && selector.UseSelected(!grenade)) { return true; }
    }
    return false;
}
