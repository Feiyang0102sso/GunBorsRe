...
CSpriteGluArchetype::CSpriteGluArchetype()
    : m_actionCount(0), m_declaredPageCount(0) {}

bool CSpriteGluArchetype::InitDrawingTree(CArrayInputStream &stream,
...
bool CSpriteGluArchetype::InitTextureMap(CArrayInputStream &stream) {
    m_rects.clear();
    m_rectIndices.clear();
...
void CSpriteGluArchetype::SetPages(std::vector<std::unique_ptr<CTexture>> &&pages) {
    m_pages = std::move(pages);
}

const AtlasRect *CSpriteGluArchetype::FindRect(std::uint16_t imageIndex) const {
...
const CTexture *CSpriteGluArchetype::GetPage(std::uint8_t page) const {
