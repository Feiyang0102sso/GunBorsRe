...
std::int16_t CPowerup::FunctionResolver(std::uint8_t function,
    ...
    if (function == 12) { return m_status.healthPercent == 100; }
    if (function == 18) { return m_status.shield; }
    if (function == 19) { return m_status.frenzy; }
    if (function == 20) { return static_cast<std::int16_t>(m_status.healthPercent); }
    if (function == 23) { return m_status.autoFire; }
    if (function == 28) {
        if (arguments[0] < 0 || arguments[0] >= 4) { ++m_unsupported; return 0; }
        return m_status.frenzyTypes[arguments[0]];
    }
