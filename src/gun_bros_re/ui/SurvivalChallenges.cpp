...
bool SurvivalHud::HasChallenges() const {
    return GameHostSettings().isConnected && m_challenges && !m_challenges->current.empty();
}

namespace {
class ChallengeCard : public IMovieRegionCallback {
public:
    ChallengeCard(MovieRenderer &renderer, const CChallengeManager::Challenge &value) : movies(renderer), challenge(value) {}
...
class ChallengeList : public IMovieRegionCallback {
public:
    ChallengeList(MovieRenderer &renderer, const CChallengeManager &manager, unsigned time, unsigned &drawn) :
    ...
    bool DrawMovieRegion(const MovieRegion &area) override {
        ...
        ChallengeCard callback(movies, challenge);
        ++rows;
...
bool SurvivalHud::DrawChallengeOverlay(float x, float y, unsigned elapsed, float alpha) {
    if (!HasChallenges()) { return true; }
    const auto id = m_movies.Ordinal("GLU_MOVIE_BRO_OPS_OVERLAY_SCROLL");
    const auto *movie = m_movies.GetMovie(id);
    unsigned start = 0, end = 0;
    if (!movie || !movie->GetChapterRange(0, start, end)) { return false; }
    ChallengeList callback(m_movies, *m_challenges, elapsed, m_challengeRows);
    return m_movies.Draw(id, start + std::min(elapsed, end - start), x, y, 1024, 768, 0, alpha, &callback);
}
