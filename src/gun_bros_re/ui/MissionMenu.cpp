...
public:
    ...
    bool DrawMovieRegion(const MovieRegion &region) override {
        ...
        if (state.missions.missionFocused < 0 && !state.missions.missionListMoving && state.mode.modePhase == 2 &&
            ...
            state.missions.waveMotion = MenuScrollMotion{};
...
void DrawMissionText(GameMenu &view, const MovieRegion &region, const std::string &text, unsigned font, bool centered ) {
...
bool DrawOriginalMissionInfo(GameMenu &view, MenuState &state, const CProfileManager &profile, bool &launched) {
    ...
    if (!state.missions.missionBound) {
        ...
        state.missions.missionMotion = MenuScrollMotion{};
    ...
    if (state.missions.missionFocused >= 0) {
        ...
        MissionCardCallbacks callbacks(view, state, profile, state.missions.missionFocused, true, launched);
