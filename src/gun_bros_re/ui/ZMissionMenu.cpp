...
public:
    ZMissionInfoCallbacks(ZGameMenu &menu, ZMenuState &menuState) : view(menu), state(menuState) {}
...
public:
    ZMissionWaveCallbacks(ZGameMenu &menu, ZMenuState &menuState, const CProfileManager &account,
    ...
    bool DrawMovieRegion(const ZMovieRegion &region) override {
        ...
        const auto *tab = FindMenuData("MDS_BUTTON_MISSION_INFO", mission.type);
        ...
        for (unsigned cell = 0; cell < 10; ++cell) {
            ...
            const auto *entry = FindMenuData("MDS_BUTTON_MISSION_WAVE", status);
            ...
            bool pressed = false;
            if (!DrawMovieButton(view, *entry, position, label, 6,
...
public:
    ZMissionCardCallbacks(ZGameMenu &menu, ZMenuState &menuState, const CProfileManager &account,
    ...
    bool DrawMovieRegion(const ZMovieRegion &region) override {
        ...
        if (region.index == 7 && region.alpha > 0) {
            ...
            for (unsigned tab = 0; tab < 3; ++tab) {
                const auto *entry = FindMenuData("MDS_BUTTON_MISSION_INFO", tabs[tab]);
            ...
            for (unsigned tab = 0; tab < 3; ++tab) {
                ZMenuDataEntry entry = *FindMenuData("MDS_BUTTON_MISSION_INFO", tabs[tab]);
                ...
                if (!DrawMovieButton(view, entry, position, view.movies.NamedString(entry.strings[0]), 5,
        ...
        if (mission.type == 2) {
            ...
            if (!locked) {
                ...
                bool pressed = false;
                if (!DrawMovieButton(view, *play, position, view.movies.NamedString(play->strings[0]), 6, true, pressed)) { return false; }
...
public:
    ZMissionListCallbacks(ZGameMenu &menu, ZMenuState &menuState, const CProfileManager &account, bool &launch)
        : view(menu), state(menuState), profile(account), launched(launch) {}
    bool DrawMovieRegion(const ZMovieRegion &region) override {
        if (region.index == 0) { return true; }
        const unsigned index = state.missions.missionFirst + region.index - 1;
        if (index >= view.planetEntries[state.planet].missions.size() || state.missions.missionFocused == static_cast<int>(index)) { return true; }
...
void DrawMissionText(ZGameMenu &view, const ZMovieRegion &region, const std::string &text, unsigned font, bool centered ) {
...
bool DrawMissionInfo(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile, bool &launched) {
    ...
    return DrawModeOverlay(view, state);
}
