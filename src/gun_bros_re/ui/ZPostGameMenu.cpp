...
bool DrawLivePostGameList(ZGameMenu &view, ZMenuState &state, const ZMovieRegion &region, bool interactive);

class ZPostGameListCallbacks : public ZMovieRegionCallback {
public:
    ZPostGameListCallbacks(ZGameMenu &menu, ZMenuState &selection, CResTOCManager &manager, ZPackTables &resources)
    ...
    bool DrawMovieRegion(const ZMovieRegion &region) override {
        ...
        for (unsigned index = first; index < std::min(3u, first + 2); ++index) {
            ...
            const auto *entry = FindMenuData("MDS_ICON_POSTGAME", icon);
            ...
            ZPostGameCardCallbacks callback(view, *entry, value, state.postGame.postGameIconTime);
...
bool DrawUpgradeButton(ZGameMenu &view, unsigned index, const ZMovieRegion &area,
    ...
    const ZMenuDataEntry *entry = FindMenuData("MDS_BUTTON_STORE_UPGRADE", index);
    if (entry == nullptr) { return false; }
    return DrawMovieButton(view, *entry, area, label, font, interactive, pressed);
}
...
bool DrawMovieButton(ZGameMenu &view, const ZMenuDataEntry &entry, const ZMovieRegion &area,
    ...
    public:
        ButtonCallback(ZGameMenu &menu, const ZMenuDataEntry &data, const std::string &caption, unsigned face,
...
bool DrawPostGame(ZGameMenu &view, ZMenuState &state, CResTOCManager &toc, ZPackTables &tables,
    ...
    public:
        MainCallbacks(ZGameMenu &menu, ZMenuState &selection, CResTOCManager &manager, ZPackTables &resources, bool enabled)
        ...
        bool DrawMovieRegion(const ZMovieRegion &region) override {
            if (region.index == 1) {
                if (state.result.live) {
                    ...
                    std::vector<const ZMenuDataEntry *> buttons{FindMenuData("MDS_BUTTON_POSTGAME_INFO", 0),
                        FindMenuData("MDS_BUTTON_POSTGAME_LEADERBOARD_MP", 0)};
                    if (state.result.deathmatch) { buttons.push_back(FindMenuData("MDS_BUTTON_POSTGAME_ADD_BRO_MP", 0)); }
                    float totalWidth = 0;
                    ...
                    for (unsigned index = 0; index < buttons.size(); ++index) {
                        ...
                        if (!DrawMovieButton(view, *buttons[index], origin, view.movies.NamedString(buttons[index]->strings[0]),
                            5, interactive && index == 0, pressed, chapter, state.postGame.postGameItemTime)) { return false; }
                        x += bounds[index].width + 2;
                    }
                    return true;
                }
                const auto *first = FindMenuData("MDS_BUTTON_POSTGAME_INFO", 0);
                ...
                for (unsigned index = 0; index < 2; ++index) {
                    const auto *entry = FindMenuData("MDS_BUTTON_POSTGAME_INFO", index);
                    if (entry == nullptr) { return false; }
                    ...
                    if (!DrawMovieButton(view, *entry, origin, view.movies.NamedString(entry->strings[0]), 5,
                ...
                const auto *entry = FindMenuData("MDS_BUTTON_POSTGAME_REPLAY_MP", index);
                if (entry == nullptr) { return false; }
                ZMovieRegion origin = region;
                ...
                if (!DrawMovieButton(view, *entry, origin, label, 1,
                ...
                const auto *entry = FindMenuData("MDS_POSTGAME_REPLAY_MP", status);
                if (entry == nullptr) { return false; }
    ...
    if (!DrawMovieButton(view, *back, position, {}, 0, ready, pressed, chapter, state.postGame.postGameItemTime, time)) { return false; }
