...
bool DrawLivePostGameList(ZGameMenu &view, ZMenuState &state, const ZMovieRegion &region, bool interactive) {
    ...
    public:
        Rows(ZGameMenu &menu, ZMenuState &result, unsigned offset) : view(menu), state(result), first(offset) {}
        bool DrawMovieRegion(const ZMovieRegion &area) override {
            ...
            const auto *entry = FindMenuData("MDS_ICON_POSTGAME_MP", icon);
            if (state.result.deathmatch) { entry = FindMenuData("MDS_ICON_POSTGAME_MP_DM", row); }
            ...
            for (unsigned peer = 0; peer < 2; ++peer) {
                ...
                ZPostGameCardCallbacks callback(view, *entry, text, state.postGame.postGameIconTime, true);
                float x = area.x;
        ...
        ZGameMenu &view; ZMenuState &state; unsigned first;
