#include "gun_bros_re/ui/ZModeOverlayCallbacks.h"
...
bool StoreItemButton(ZGameMenu &view, unsigned entryIndex, float right, float y, float height, bool enabled, float alpha ) {
    const ZMenuDataEntry *entry = FindMenuData("MDS_BUTTON_STORE_ITEMS", entryIndex);
...
void DrawStoreTemplate(ZGameMenu &view, const std::string &text, const ZMovieRegion &area,
    ...
    float y = area.y;
    for (const ZStoreTextLine &line : lines) {
        float x = area.x;
        if (centered) { x += (layoutWidth - line.width) / 2; }
        for (const ZStoreTextRun &run : line.runs) {
...
void DrawStoreCategories(ZGameMenu &view, const ZMovieRegion &bar, ZMenuState &state, bool interactive) {
    ...
    for (unsigned category = 0; category < 4; ++category) {
        const ZMenuDataEntry *entry = FindMenuData("MDS_BUTTON_STORE_CATEGORIES", category);
...
unsigned StoreFilterRows(unsigned category, const char *&table) {
    ...
    unsigned count = 0;
    while (FindMenuData(table, count)) { ++count; }
    return count;
}
...
bool StoreGunSwapOrigin(ZGameMenu &view, const ZMovieRegion &parent, ZMovieRegion &origin) {
    const ZMenuDataEntry *entry = FindMenuData("MDS_BUTTON_STORE_GUN_SWAP", 0);
...
bool DrawStoreGunSwap(ZGameMenu &view, ZMenuState &state, const ZMovieRegion &parent, bool interactive) {
    ...
    if (!DrawMovieButton(view, *entry, origin, std::to_string(view.GetPlayerPreviewSlot() + 1), 6,
...
bool DrawStorePrompt(ZGameMenu &view, ZMenuState &state) {
    ...
    const auto *entry = FindMenuData(state.storePromptTable, state.storePromptIndex);
    ...
    float textHeight = 0;
    for (const ZStoreTextLine &line : lines) { textHeight += line.height; }
    ...
    if (state.storePromptDismiss) {
        ...
        const auto *dismiss = FindMenuData("MDS_BUTTON_POPUP_PROMPT", 0);
    ...
    if (state.storePromptButtons != nullptr) {
        ...
        for (unsigned index = first; index <= last; ++index) {
            ...
            const auto *button = FindMenuData(state.storePromptButtons, index);
            ...
            bool pressed = false;
            if (!DrawMovieButton(view, *button, placed, view.movies.NamedString(button->strings[0]), 5,
...
bool DrawCurrencyCard(ZGameMenu &view, CResTOCManager &toc, ZPackTables &tables,
    ...
    const ZMenuDataEntry *entry = FindMenuData("MDS_BUTTON_STORE_ITEMS", action);
    ZMovieRegion button;
...
bool DrawStore(ZGameMenu &view, CResTOCManager &toc, ZPackTables &tables, CProfileManager &profile,
    ...
    for (const auto &slot : view.movies.Regions(storeScroll, view.storeRestTime)) {
        ...
        for (unsigned row = 0; row < 2; ++row) {
            ...
            ZMovieRegion name, icon, kind, price, right;
            ...
            const ZWeaponEntry *cardWeapon = FindWeaponEntry(weapons, ref.object);
            ...
            const ZMenuDataEntry *actionEntry = FindMenuData("MDS_BUTTON_STORE_ITEMS", action);
    ...
    if (view.movies.Region(sortBar, kSortPanelRegion, sortTime, sortPanel)) {
        ...
        for (unsigned row = 0; row < rows; ++row) {
            const ZMenuDataEntry *entry = FindMenuData(table, row);
            if (entry == nullptr) { return false; }
    ...
    if (state.store.shopDetailOpen && state.selectedItem >= 0 && state.selectedItem < static_cast<int>(store.size())) {
        ...
        ZStoreCardFace open = face;
        open.time = openStart;
        ZMovieRegion finalDescription, finalStats;
        if (!CardRegion(view, shopBox, kCardDescriptionRegion, open, finalDescription) ||
            !CardRegion(view, shopBox, kCardRightRegion, ZStoreCardFace{face.x, face.y, 1, cardStart}, finalStats)) { return false; }
        ...
        const ZWeaponEntry *weapon = FindWeaponEntry(weapons, ref.object);
        unsigned mastery = 0;
        ...
        if (CardRegion(view, shopBox, kCardActionRegion, face, region) && region.alpha > 0) {
            ...
            const ZMenuDataEntry *previewEntry = FindMenuData("MDS_BUTTON_STORE_PREVIEW", 0);
...
bool DrawModeOverlay(ZGameMenu &view, ZMenuState &state) {
    ...
    ZModeOverlayCallbacks callback(view, state, otherAlpha);
