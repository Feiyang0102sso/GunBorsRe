...
bool CPowerUpSelector::DrawSelectorButton(const char *table, unsigned index, float x, float y, float alpha,
    ...
    public:
        Callback(CPowerUpSelector &owner, const ZMenuDataEntry &data) : hud(owner), entry(data) {}
...
bool CPowerUpSelector::DrawSelectorItem(const ZInputPadState &state, unsigned index, const ZMovieRegion &area) {
    ...
    const auto *buy = FindMenuData("MDS_BUTTON_POWERUP_SELECTOR_BUY", 0);
    ...
    public:
        ...
        bool DrawMovieRegion(const ZMovieRegion &area) override {
            ...
            if (area.index == 3) {
                ...
                const auto *entry = FindMenuData("MDS_BUTTON_POWERUP_SELECTOR", cancelIndex);
                if (entry == nullptr) { return false; }
        ...
        public:
            ...
            bool DrawMovieRegion(const ZMovieRegion &area) override {
                if (area.index == 0 || area.index == 1) {
                    const auto *entry = FindMenuData("MDS_BUTTON_POWERUP_SELECTOR", 2);
                ...
                if (area.index == 5) {
                    ...
                    const auto *entry = FindMenuData("MDS_BUTTON_POWERUP_SELECTOR", 3);
                    const float x = area.x + area.width / 2, y = area.y + area.height / 2;
                    float alpha = area.alpha;
                    if (!use) {
                        const unsigned sprite = entry->sprites[0];
                    ...
                    const auto lines = FormatStoreText(hud.m_resources.m_movies, hud.m_resources.m_movies.NamedString(entry->strings[0]), 100, {5,5,5,5,5});
...
void CPowerUpSelector::ReportSelectorPurchase(ZPurchaseResult result, const ZInputPadState &state) {
    ...
    const auto *entry = FindMenuData(m_selectorPromptTable, 0);
...
bool CPowerUpSelector::DrawSelectorPrompt() {
    ...
    const auto lines = FormatStoreText(m_resources.m_movies, body, compact.width, {0,1,0,0,0});
    ...
    if (m_selectorPromptFunds) {
        for (unsigned index = 1; index <= 2; ++index) {
            ...
            if (m_selectorPrompt.IsReady()) {
                m_selectorPromptHits.push_back({touch, FindMenuData("MDS_BUTTON_STORE_INGAME_PROMPT", index)->action});
            }
        }
    } else {
