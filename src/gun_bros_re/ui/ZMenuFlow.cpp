...
int ShowGameMenu(CResTOCManager &toc, ZPackTables &tables, CProfileManager &profile,
    ...
    const std::string &capturePath, const std::vector<ZMenuTestClick> *testClicks , bool originalProfile , ZWindow *sharedWindow , bool testTransitions , ZMenuTransitionTrace *transitionTrace , CBGM *sharedMusic ) {
    // CGunBros owns CBGM across loading, gameplay and postgame menus.
    ZGameMenu view(sharedWindow);
    ...
    while (view.window.PumpEvents()) {
        ...
        for (ZKeyCode key = view.window.TakeKeyPress(); key != ZKeyCode::None; key = view.window.TakeKeyPress()) {
            ...
            if (state.page == 0 && key == ZKeyCode::Down) { state.planet = (state.planet + 1) % 4; }
            if (state.page == 0 && key == ZKeyCode::Up) { state.planet = (state.planet + 3) % 4; }
            ...
            if (key == ZKeyCode::Q && state.page == 2) { state.store.shopSwapKeyRequested = true; }
        }
        ...
        if (state.page == 27 || state.page == 28) {
            if (profile.nativeArchive) {
                if (!DrawPostGame(view, state, toc, tables, profile)) { return -3; }
            } else { return -3; }
        }
        if (state.page == 24 && profile.nativeArchive) {
            if (!DrawGreeting(view, state, toc, tables, profile, daily, store, progress, savePath, now)) { return -3; }
        }
        if (state.page == 0 || state.page == 22) {
            ...
            if (!DrawStarMap(view, state, profile) || !DrawModeOverlay(view, state)) { return -3; }
        }
        if (state.page == 21) {
            if (!profile.nativeArchive) { return -3; }
            bool launch = false;
            if (!DrawMissionInfo(view, state, profile, launch)) { return -3; }
            if (launch) {
        ...
        if (state.page == 25 || state.page == 29) {
            ...
            if (!DrawPlayerSelect(view, state, profile, savePath, launchTutorial)) { return -3; }
            ...
            if (!DrawSocialMenu(view, state, profile, credentials)) { return -3; }
