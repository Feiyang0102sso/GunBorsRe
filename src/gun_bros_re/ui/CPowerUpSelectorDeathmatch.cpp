    ...
    public:
        ...
        bool DrawMovieRegion(const ZMovieRegion &region) override {
            ...
            if (region.index == 2) {
                ...
                const auto lines = FormatStoreText(hud.m_resources.m_movies, description, region.width, {1,1,1,1,0});
                float y = region.y;
...
bool CPowerUpSelector::DrawMatchTabs(const ZMovieRegion &area) {
    ...
    for (unsigned index = 0; index < 2; ++index) {
        const auto *entry = FindMenuData("MDS_BUTTON_POWERUP_SELECTOR", index + 4);
    ...
    for (unsigned index = 0; index < 2; ++index) {
        ...
        public:
            Label(CPowerUpSelector &owner, const ZMenuDataEntry &item, bool active) : hud(owner), entry(item), selected(active) {}
            bool DrawMovieRegion(const ZMovieRegion &region) override {
            ...
            CPowerUpSelector &hud; const ZMenuDataEntry &entry; bool selected;
