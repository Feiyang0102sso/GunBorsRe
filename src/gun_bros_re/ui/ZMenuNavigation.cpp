...
int ZGameMenu::Header(const CProfileManager &profile, const CPlayerProgress &progress, unsigned currentPage) {
    ...
    public:
        InfoCallback(ZGameMenu &menu, const CPlayerProgress &experience) : view(menu), progress(experience) {
            char digits[16];
    ...
    public:
        HeaderCallback(ZGameMenu &menu, const CProfileManager &account, InfoCallback &cluster,
        ...
        bool DrawMovieRegion(const ZMovieRegion &region) override {
            if (region.index < std::size(kNavigationBranches)) {
                ...
                const auto *entry = FindMenuData("MDS_BUTTON_TRUNK", branch - 1);
                ...
                bool pressed = false;
                if (!DrawMovieButton(view, *entry, origin, "", 0, enabled, pressed,
                ...
                const auto *entry = FindMenuData("MDS_BUTTON_TRUNK", kNavigationBranches[index] - 1);
                if (entry == nullptr) { return false; }
