...
bool DrawLivePostGameList(GameMenu &view, MenuState &state, const MovieRegion &region, bool interactive) {
    ...
    class Rows : public IMovieRegionCallback {
    public:
        Rows(GameMenu &menu, MenuState &result, unsigned offset) : view(menu), state(result), first(offset) {}
        ...
        GameMenu &view; MenuState &state; unsigned first;
