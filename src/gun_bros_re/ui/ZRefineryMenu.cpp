...
bool StartRefineryTransfer(ZGameMenu &view, ZMenuState &state, CProfileManager &profile,
    ...
    if ((record.state == 1 || (record.state == 3 && state.refinery.refineryStatusChapter[slot] == 3))) {
        if (record.state == 3 || profile.xplodium != 0) {
            ...
            if (record.state == 3) {
                ...
                const auto *image = FindMenuData("MDS_ICON_STANDARD", 0);
                ZMovieRegion bounds;
            ...
            const auto *image = FindMenuData("MDS_ICON_STANDARD", icon);
...
public:
    ZRefineryCallbacks(ZGameMenu &menu, ZMenuState &selection, CProfileManager &account,
    ...
    bool Categories(const ZMovieRegion &region) {
        ...
        const auto *first = FindMenuData("MDS_BUTTON_REFINE_SLOT_CATEGORY", 1);
        ...
        for (unsigned position = 0; position < 2; ++position) {
            const auto *entry = FindMenuData("MDS_BUTTON_REFINE_SLOT_CATEGORY", 1 - position);
            ...
            if (!DrawMovieButton(view, *entry, area, view.movies.NamedString(entry->strings[0]), 5,
    ...
    bool Xplodium(const ZMovieRegion &region) {
        const auto *entry = FindMenuData("MDS_ICON_STANDARD", 0);
        if (entry == nullptr) { return false; }
    ...
    bool Meter(const ZMovieRegion &region) {
        ...
        if (!chamber.opening) {
            ...
            if (!DrawMovieButton(view, *entry, button, {}, 0,
        ...
        if (record.state == 0 && !profile.refinery.IsGated(slot)) {
            ...
            const ZMenuDataEntry unlock{"MDC_REFINE_UNLOCK", slot, {"", "", "", ""},
            ...
            bool pressed = false;
            if (!DrawMovieButton(view, unlock, button, view.movies.NamedString("IDS_SHOP_BUY"), 5, interactive && state.refinery.refineryTransfer < 0,
...
bool DrawSocialOffline(ZGameMenu &view, ZMenuState &state, bool hasCredentials) {
    ...
    const auto *entry = FindMenuData(table, description);
    ...
    for (const auto &region : view.movies.Regions(ordinal, state.social.socialTime)) {
        if (region.index == 0) {
            bool pressed = false;
            if (!DrawMovieButton(view, *button, region, view.movies.NamedString(button->strings[0]), 6, true, pressed)) { return false; }
            if (pressed) {
