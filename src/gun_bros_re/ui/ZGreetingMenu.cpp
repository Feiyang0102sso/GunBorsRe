...
public:
    ZGreetingCallbacks(ZGameMenu &menu, ZMenuState &state, CResTOCManager &toc, ZPackTables &tables,
    ...
    bool DrawMovieRegion(const ZMovieRegion &region) override {
        if (region.index <= 2) {
            const auto *entry = FindMenuData("MDS_GREETING_STRINGS", region.index);
        ...
        if (region.index == 3 || region.index == 4) {
            const auto *entry = FindMenuData("MDS_BUTTON_GREETING_REDIRECTS", region.index - 3);
            if (entry == nullptr) { return false; }
            ZMovieRegion origin = region;
            origin.x += region.width / 2;
            origin.y += region.height / 2;
            bool pressed = false;
            if (!DrawMovieButton(view, *entry, origin, {}, 0, interactive, pressed)) { return false; }
            if (pressed) {
        ...
        if (region.index == 5 || region.index == 7) {
            ...
            const auto *entry = FindMenuData("MDS_OFFLINE_GREETING", index);
            if (entry == nullptr) { return false; }
