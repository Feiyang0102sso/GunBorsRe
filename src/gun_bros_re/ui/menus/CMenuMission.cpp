...
class ZStarPlanetCallbacks : public ZMovieRegionCallback {
public:
    ZStarPlanetCallbacks(ZMenuSurface &menu, CMenuSystem &state) : view(menu), state(state) {}
    bool DrawMovieRegion(const ZMovieRegion &region) override {
        if (region.index == 0) { return true; }
        int index = view.planets.PlanetForMapSlot(region.index);
        if (index < 0) { index = view.planets.PlanetForMapSlot(0); }
        if (index < 0) { return false; }
        const float fade = std::min(1.0f, state.starMap.starFadeTime / 350.0f); // OnShow :162368.
        bounds.push_back(view.planets.DrawPlanetThumb(index, region, fade));
        return true;
    }
    ZMenuSurface &view;
    CMenuSystem &state;
...
bool CMenuMission::Draw(ZMenuSurface &view, CMenuSystem &state, const CProfileManager &profile) {
    ...
    bool interactive = state.stack.page == 0 && state.mode.modeSelected && state.mode.modePhase == 2 && !state.starMap.starEntering && state.starMap.starFadeTime == 350;
    ZMovieRegion modeButton;
    ...
    if (interactive && view.MouseIn(viewport.x, viewport.y, viewport.width, viewport.height)) {
        ...
        if (view.dragX != 0 && elapsed != 0) {
            ...
            unsigned chapter = 0;
            while (chapter + 1 < map->chapter.starts.size() && map->chapter.starts[chapter + 1] <= state.starMap.starTime) { ++chapter; }
            if (wheel < 0 && chapter + 1 < map->chapter.starts.size()) { ++chapter; }
            if (wheel > 0 && chapter > 0) { --chapter; }
            state.starMap.starTargetTime = map->chapter.starts[chapter];
        }
    }
    ...
    ZStarPlanetCallbacks callbacks(view, state);
    ...
    for (unsigned slot = 1; slot <= map->chapter.starts.size(); ++slot) {
    ...
    int selected = view.planets.PlanetForMapSlot(state.starMap.starSelectedSlot);
    if (selected < 0) { selected = view.planets.PlanetForMapSlot(0); }
    ...
    if (state.starMap.starLocked && !state.starMap.starEntering && state.starMap.starSelectorX >= 0 && state.starMap.starSelectorX <= kMenuWidth) {
        ZMovieRegion flagBounds, reticleBounds;
        ...
        for (const auto &region : view.movies.Regions(flagOrdinal, state.starMap.starFlagTime, state.starMap.starSelectorX + state.starMap.starFlagX, state.starMap.starSelectorY + state.starMap.starFlagY)) {
            if (region.index == 0) { view.planets.PlanetFlagLines(state.starMap.starSelectorX, state.starMap.starSelectorY, region); }
            ...
            const unsigned requiredLevel = view.planets.planetEntries[selected].data.requiredLevel;
            ...
            std::string label = view.planets.names[selected];
            if (region.index == 3) {
            ...
            const auto lines = CTextBox::Format(view.movies, label, textArea.width, {1, 2, 2, 2, 2});
            float height = 0;
            for (const auto &line : lines) { height += line.height; }
            float y = textArea.y + (textArea.height - height) / 2;
            ZStoreRegionClip clip(view, textArea);
    ...
    if (state.starMap.starEntering && state.starMap.starReticleTime == closeEnd) {
        ...
        if (state.gameMode == 2 && !view.planets.planetEntries[selected].data.object12.IsNull()) {
            if (!BeginLocalMatch(state)) { return false; }
        } else if (!view.planets.planetEntries[selected].missions.empty()) { state.Navigate(21); }
