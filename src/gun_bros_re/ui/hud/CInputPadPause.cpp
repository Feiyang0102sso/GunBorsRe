...
bool CInputPad::DrawPause(const ZInputPadState &state) {
    ...
    if (!m_pauseBound) {
        ...
        for (unsigned index = 0; CMenuDataProvider::Find(table, index) != nullptr; ++index) {
            if (!m_pauseHelp && CMenuDataProvider::Find(table, index)->action == 135) { continue; }
    ...
    const auto lines = CTextBox::Format(m_resources.m_movies, PauseText(state, m_pauseItems[m_pauseFocus], 1), content.width - scrollBar.width, {0, 6, 0, 0, 0});
