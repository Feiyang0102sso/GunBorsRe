...
#ifndef GUN_BROS_RE_ZLOADINGSCREEN_H
...
public:
    ZLoadingScreen(ZWindow &window, ZMovieRenderer &movies, CGunBros &tables, const CProfileManager *profile = nullptr, bool enteringGame = false, bool startup = false, CBGM *music = nullptr, bool multiplayer = false, bool deathmatch = false)
        ...
        static unsigned nextMultiplayerIndex = 0;
        unsigned index = nextIndex;
        ZLoadingMode mode = ZLoadingMode::Solo;
        if (multiplayer) { index = nextMultiplayerIndex++; mode = ZLoadingMode::Live; }
        if (deathmatch) { index = nextMultiplayerIndex++; mode = ZLoadingMode::Deathmatch; }
...
private:
    ...
    bool Draw(unsigned elapsed, bool playChapter = false, const std::string &capturePath = {}) {
        ...
        bool captured = true;
        if (!capturePath.empty()) { captured = Capture::SaveFrame(m_window, capturePath); }
        m_window.Present();
