...
namespace {
ZLocalBotFriend *MatchBot(CMenuSystem &state) {
...
int RunFrontEndSurvival(CGame::Launch launch, CMenuSystem &state) {
...
int RunGameMenuSession(const std::string &bigDirectory, const std::string &screenshotPath, unsigned page, bool originalProfile,
    ...
    state.stack.page = std::min(page, 29u);
    ...
    if (state.stack.page == 1) { state.stack.page = 2; }
    if (state.stack.page == 7 || state.stack.page == 20 || state.stack.page == 16) { state.stack.page = 0; }
    if (state.stack.page == 9 || state.stack.page == 10) { state.stack.page = 4; }
    if (state.stack.page == 12) { state.stack.page = 6; state.settings.optionsFocus = 9; }
    if (state.stack.page == 15) { state.stack.page = 0; }
    if (state.stack.page == 19 || state.stack.page == 23) { state.stack.page = 21; }
    while (true) {
        ...
        if (choice == 5) {
            ...
            if (profile.tutorialCompleted) { state.postGame.Refresh(state, context, weapons); }
        ...
        GameObjectRef level;
        if (profile.nativeArchive) {
            level = profile.nativeArchive->survivalLevels.at(choice);
        } else {
