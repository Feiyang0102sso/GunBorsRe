...
public:
    ZSocialContentCallback(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile,
    ...
    bool ChallengeSprite(unsigned entryIndex, bool active, float x, float y, float alpha) {
        ...
        const auto *entry = FindMenuData("MDS_ICON_CHALLENGES", entryIndex);
    ...
    bool DrawMovieRegion(const ZMovieRegion &region) override {
        ...
        if (part == ZSocialPart::Endcap && region.index == 2) {
            ...
            const auto *entry = FindMenuData("MDS_FRIENDS_ENDCAP", 0);
            if (!entry) { return false; }
            return view.movies.DrawSprite(entry->sprites[0] >> 16, entry->sprites[0] & 255,
                0, region.x, region.y, 1, region.alpha);
        }
        if (part != ZSocialPart::Challenge && part != ZSocialPart::Details) { return true; }
        ...
        if (region.index == 2) {
            const auto *button = FindMenuData("MDS_BUTTON_CHALLENGE_INVITE_BRO", 0);
            if (!button) { return false; }
            bool pressed = false;
            if (!DrawMovieButton(view, *button, region, view.movies.NamedString(button->strings[0]),
...
bool DrawPart(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile,
    ...
    if (part == ZSocialPart::Challenge) { time = state.social.challengeTimes[index]; }
    ...
    if (part == ZSocialPart::Details) { time = state.social.sidebarTime; }
...
public:
    ZSocialListCallback(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile, unsigned first) :
...
bool DrawSocialContent(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile, const ZMovieRegion &region) {
    ...
    if (state.page == 5 && region.index == 3 && social.socialTab == 0) {
        ...
        return DrawPart(view, state, profile, "GLU_MOVIE_BRO_OPS_DETAILS", ZSocialPart::Details,
