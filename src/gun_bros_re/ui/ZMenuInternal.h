...
struct ZMenuState {
    ...
    ZStarMapMenuState starMap;
    ZStoreMenuState store;
    ZMissionsMenuState missions;
    ZModeMenuState mode;
    ZPostGameMenuState postGame;
    ZRefineryMenuState refinery;
    ZGreetingMenuState greeting;
    ZSelectionMenuState selection;
    ZSettingsMenuState settings;
    ZSocialMenuState social;
...
struct ZMenuTestClick { float x; float y; unsigned advanceMs = 0; unsigned renderDelayMs = 0; };

/** All GL owners are destroyed before the menu window's context. */
class ZGameMenu {
public:
    explicit ZGameMenu(ZWindow *sharedWindow = nullptr) : window(sharedWindow ? *sharedWindow : ownedWindow) {}
    /** Integration harness input; it still goes through rendered button hit tests. */
    void SetTestClick(const ZMenuTestClick &click) { mouseX = click.x; mouseY = click.y; clicked = true; }
    ...
    bool DrawEquippedPlayer(CResTOCManager &toc, ZPackTables &tables, const CProfileManager &profile,
...
bool RequireRegion(ZGameMenu &view, unsigned movie, unsigned index, unsigned time, ZMovieRegion &region, const char *what);

/** SHOP_BOX regions resolved for a card whose own origin sits at (x, y). */
bool CardRegion(ZGameMenu &view, unsigned card, unsigned index, const ZStoreCardFace &face, ZMovieRegion &region);
...
void PlateLabel(ZGameMenu &view, const std::string &label, float x, float y, float width, float height);
...
bool StoreItemButton(ZGameMenu &view, unsigned entryIndex, float right, float y, float height, bool enabled, float alpha = 1);
...
std::string StoreCostText(ZGameMenu &view, const CStoreItem &item);

void DrawCardPrice(ZGameMenu &view, const CStoreItem &item, const ZMovieRegion &row, float alpha);
...
const ZWeaponEntry *FindWeaponEntry(const std::vector<ZWeaponEntry> &weapons, const GameObjectRef &ref);
...
std::string StoreItemKind(ZGameMenu &view, const ZStoreEntry &item);
...
void DrawStoreTemplate(ZGameMenu &view, const std::string &text, const ZMovieRegion &area,
...
bool DrawMasteryMeter(ZGameMenu &view, const ZWeaponEntry &weapon, unsigned experience,
...
bool DrawPowerupCompatibility(ZGameMenu &view, const CStoreItem &item, const ZMovieRegion &area, unsigned elapsed);
...
void DrawStoreCategories(ZGameMenu &view, const ZMovieRegion &bar, ZMenuState &state, bool interactive);
...
bool AdvanceStoreFilter(ZGameMenu &view, ZMenuState &state, const CMovie &movie);
...
void DrawStoreQuantity(ZGameMenu &view, const CProfileManager &profile, const GameObjectTypeRef &ref,
...
bool AdvanceStoreCard(ZGameMenu &view, ZMenuState &state, const CMovie &movie);
...
bool StoreFailureText(ZGameMenu &view, const ZMenuState &state, std::string &body);

void ShowStoreFundsPrompt(ZMenuState &state, const std::vector<ZStoreEntry> &store,
    const CProfileManager &profile, unsigned currency, unsigned price, bool inGame = true);

bool DrawMovieButton(ZGameMenu &view, const ZMenuDataEntry &entry, const ZMovieRegion &area,
...
bool StoreGunSwapOrigin(ZGameMenu &view, const ZMovieRegion &parent, ZMovieRegion &origin);

/** Original button chapters own appearance, press completion and category hide. */
bool DrawStoreGunSwap(ZGameMenu &view, ZMenuState &state, const ZMovieRegion &parent, bool interactive);

bool CompleteOfflineIAP(std::uint64_t clock, ZMenuState &state, CProfileManager &profile,
...
bool DrawStorePrompt(ZGameMenu &view, ZMenuState &state);
...
bool DrawCurrencyCard(ZGameMenu &view, CResTOCManager &toc, ZPackTables &tables,
    const ZStoreEntry &item, unsigned index, unsigned movie, const ZStoreCardFace &face,
    bool enabled, ZMenuState &state, CProfileManager &profile, const std::filesystem::path &savePath);
 ...
 */
bool EquipStoreItem(CProfileManager &profile, const CStoreItem &item, const std::vector<ZArmorEntry> &armors);

bool DrawStore(ZGameMenu &view, CResTOCManager &toc, ZPackTables &tables, CProfileManager &profile,
    unsigned level, const std::vector<ZStoreEntry> &store, const std::vector<ZWeaponEntry> &weapons,
    const std::vector<ZArmorEntry> &armors, ZMenuState &state, const std::filesystem::path &savePath);
...
bool DrawModeOverlay(ZGameMenu &view, ZMenuState &state);
...
bool DrawStarMap(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile);
...
void DrawMissionText(ZGameMenu &view, const ZMovieRegion &region, const std::string &text, unsigned font, bool centered = false);
...
bool ScrollMissionMovie(ZGameMenu &view, ZMenuScrollMotion &motion, float &position,
...
bool DrawMissionInfo(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile, bool &launched);
...
void CloseMastery(ZMenuState &state);
...
unsigned MasteryMeterTime(ZGameMenu &view, const ZWeaponEntry &weapon, unsigned experience);

/** Bind CMenuMovieButton's original region 1 graphic/label and region 0 hit box. */
bool DrawUpgradeButton(ZGameMenu &view, unsigned index, const ZMovieRegion &area,
    const std::string &label, unsigned font, bool interactive, bool &pressed);
...
void UpgradeCenteredText(ZGameMenu &view, const ZMovieRegion &region, const std::string &text, unsigned font);
...
void DrawUpgradeStats(ZGameMenu &view, const ZMovieRegion &area, const CStoreItem &item, unsigned level, bool next);
...
bool DrawMastery(ZGameMenu &view, ZMenuState &state, CProfileManager &profile, CResTOCManager &toc,
...
std::string PostGameFormat(ZGameMenu &view, const char *name, const std::vector<std::string> &values);
...
bool DrawPostGame(ZGameMenu &view, ZMenuState &state, CResTOCManager &toc, ZPackTables &tables,
...
std::string RefineryNumber(ZGameMenu &view, const char *name, std::uint64_t value);

bool SetRefineryStatus(ZGameMenu &view, ZMenuState &state, unsigned slot, unsigned chapter);
...
bool DrawRefinery(ZGameMenu &view, ZMenuState &state, CProfileManager &profile,
...
bool DrawRefineryOverlay(ZGameMenu &view, const ZMenuState &state);
...
bool DrawSocialOffline(ZGameMenu &view, ZMenuState &state, bool hasCredentials);
bool DrawSocialMenu(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile, bool hasCredentials);
bool BindSocialContent(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile);
bool DrawSocialContent(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile, const ZMovieRegion &region);
bool DrawSocialModel(ZGameMenu &view, ZMenuState &state, const CProfileManager &profile);
...
std::string OptionsText(ZGameMenu &view, const CProfileManager &profile, unsigned index, unsigned slot, const char *table = "MDS_OPTIONS");
...
bool DrawGreeting(ZGameMenu &view, ZMenuState &state, CResTOCManager &toc, ZPackTables &tables,
...
int ShowGameMenu(CResTOCManager &toc, ZPackTables &tables, CProfileManager &profile,
    ...
    const std::vector<ZArmorEntry> &armors, ZMenuState &state, const std::filesystem::path &savePath,
    const std::string &capturePath, const std::vector<ZMenuTestClick> *testClicks = nullptr, bool originalProfile = false, ZWindow *sharedWindow = nullptr, bool testTransitions = false, ZMenuTransitionTrace *transitionTrace = nullptr, CBGM *sharedMusic = nullptr);
