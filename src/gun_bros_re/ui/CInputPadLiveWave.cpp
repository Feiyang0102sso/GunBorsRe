...
void CInputPad::BeginLiveWave(const ZMultiplayerStatistics &player, const ZMultiplayerStatistics &peer) {
    ...
    QueueNotice("GLU_MOVIE_WAVE_WRAPUP", "", "", true);
}

bool CInputPad::DrawLiveWave(unsigned time) {
    class Callback : public ZMovieRegionCallback {
    public:
        Callback(CInputPad &owner) : hud(owner) {}
        bool DrawMovieRegion(const ZMovieRegion &area) override {
            ...
            if (area.index == 2 || area.index == 3) {
                ...
                return hud.m_resources.m_movies.DrawSprite(0, animation, 0, area.x, area.y, 1, area.alpha);
            }
            if (area.index == 4 && hud.m_liveWaveRemaining < 7000 && hud.m_liveWaveRemaining >= 2000) {
                ...
                text = hud.m_resources.m_movies.NamedString("IDS_MULTIPLAYER_WRAPUP_WAVE_START_ENDLESS") +
            ...
            if (area.index >= 5 && area.index <= 10) { text = hud.m_resources.m_movies.NamedString(labels[area.index - 5]); }
            ...
            return hud.m_resources.m_movies.Text(text, area.x + (area.width - hud.m_resources.m_movies.TextWidth(text, font)) / 2,
                area.y + (area.height - hud.m_resources.m_movies.TextHeight(font)) / 2, font, 1, 0, area.alpha);
        }
        CInputPad &hud;
    } callback(*this);
    return m_resources.m_movies.Draw(m_resources.m_movies.Ordinal("GLU_MOVIE_WAVE_WRAPUP"), time, 512, 384, 1024, 768, 0, 1, &callback);
}
