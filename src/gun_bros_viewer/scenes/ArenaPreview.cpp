...
int RunArena(const std::string &bigDirectory, std::uint32_t enemyIndex,
    ...
    // CRenderQueue consumes the same map-owned player record as the game.
    // The presentation map deliberately contains no tiles or placed scenery.
    CMap presentation;
    auto &presentationResources = presentation.GetResources();
    presentationResources.playerTemplate = std::make_unique<CBrother::Template>(playerData);
    presentationResources.players.emplace_back();
    auto &presentedPlayer = presentationResources.players.front();
    presentedPlayer.model = std::make_unique<CBrother>();
    CBrother &player = *presentedPlayer.model;
    ZQuadBatch sceneBatch;
    if (!sceneBatch.Create(program)) { return 1; }
    ...
    ZArenaContext context;
    auto scene = std::make_unique<CLevel>(toc, tables, program);
    ...
    if (!context.Load(toc, tables) || !context.Bind(tables, *scene, catalog[entry])) { return 1; }
    ...
    while (controls.PumpEvents()) {
        ...
        for (ZKeyCode key = controls.TakeKeyPress(); key != ZKeyCode::None; key = controls.TakeKeyPress()) {
            if (controls.IsPressed(key, ViewerAction::Previous)) { nextEntry = (nextEntry + catalog.size() - 1) % catalog.size(); }
                ...
                if (context.SpawnNearby(*scene, entry, catalog[entry]) == nullptr) { noticeUntil = window.GetTicksMs() + 2500; }
            }
        ...
        if (nextEntry != entry || reset) {
            entry = nextEntry;
            // A fresh level cannot retain the previous entry's Flow callbacks,
            // multipliers or object limits when the next entry has no LEVEL.
            // The persistent brother must stop borrowing the old level before
            // its replacement resets/equips the player and runs PLAYER exports.
            player.SetLevelContext(nullptr);
            scene = std::make_unique<CLevel>(toc, tables, program);
            scene->BindCombat(catalog, player, vitals, playerData.GetGameScale());
            if (!context.Bind(tables, *scene, catalog[entry])) { return 1; }
        ...
        // The original queue now owns per-model depth and all three projectile
        // passes, sorted with actors (CRenderQueue::Draw :145123).
        presentedPlayer.x = scene->GetPlayer().x;
        presentedPlayer.y = scene->GetPlayer().y;
        presentedPlayer.facingDegrees = scene->GetPlayer().facing;
        CRenderQueue::Draw(presentation, sceneBatch, program, projection, false, scene.get(),
            nullptr, 0, arenaWidth);
        scene->Draw(projection, nullptr, MapDetail::kLevelCameraScale, true);
        glDisable(GL_DEPTH_TEST);
        ...
        if (collisions) {
            DrawCollisionOverlay(markers, markerProgram, projection, 1 / camera.scale, nullptr, scene.get(), nullptr, scene.get());
        }
        ...
        controls.DrawLabel(context.GetLabel(), 16, kInfoHeight + 36, hudWidth - 32, fontHeight, hudProjection);
        if ((deferred & 3) != 0) { controls.DrawLabel("Boss / level mechanisms deferred", 16, kInfoHeight + 64, hudWidth - 32, fontHeight, hudProjection); }
