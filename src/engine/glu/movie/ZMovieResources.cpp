...
bool ZMovieRenderer::Init(CResPackTOC &pack, CResPackTOC &core) {
    m_pack = &pack;
    m_core = &core;
...
CMovie *ZMovieRenderer::GetMovie(unsigned ordinal) {
...
unsigned ZMovieRenderer::SpriteDuration(unsigned archetype, unsigned animationIndex) {
    Animation *animation = GetAnimation(archetype, animationIndex);
    if (animation == nullptr) { return 0; }
    return animation->duration;
}

bool ZMovieRenderer::SpriteBounds(unsigned archetype, unsigned animationIndex, ZMovieRegion &bounds) {
    Animation *animation = GetAnimation(archetype, animationIndex);
...
std::string ZMovieRenderer::NamedString(const char *name, unsigned offset) {
...
bool ZMovieRenderer::BindSpritePlayer(unsigned archetype, unsigned animationIndex, CSpritePlayer &player) {
    Animation *animation = GetAnimation(archetype, animationIndex);
    if (animation == nullptr || animation->steps.empty()) { return false; }
...
bool ZMovieRenderer::DrawSpritePlayer(unsigned archetype, unsigned animationIndex, const CSpritePlayer &player,
