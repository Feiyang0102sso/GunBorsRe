/** @file ZMovieRenderer.h
 * @brief Draw original CMovie resources and expose their interactive regions.
 */
#ifndef GUN_BROS_RE_ZMOVIERENDERER_H
#define GUN_BROS_RE_ZMOVIERENDERER_H
...
public:
    ...
    virtual bool DrawMovieRegion(const ZMovieRegion &region) = 0;
};
...
public:
    ...
    bool Region(unsigned ordinal, unsigned index, unsigned time, ZMovieRegion &region);
    bool Draw(unsigned ordinal, unsigned time, float x = 512, float y = 384, float width = 1024, float height = 768,
        unsigned depth = 0, float alpha = 1, ZMovieRegionCallback *callback = nullptr);
    ...
    bool SpriteBounds(unsigned archetype, unsigned animation, ZMovieRegion &bounds);
    void Image(const ZTexture &texture, float x, float y, float width, float height, bool flipVertical = false);
    ...
    std::vector<ZMovieRegion> Regions(unsigned ordinal, unsigned time, float x = 512, float y = 384, bool includeInvisible = false);
...
private:
    ...
    struct Animation {
        std::vector<std::vector<ZSpriteQuad>> steps;
    ...
    ZMovieKeyFrame AtTime(const CMovieObject &object, unsigned time) const;
    Metrics GetMetrics(const CMovie &movie, unsigned object, unsigned time, float width, float height, unsigned depth = 0);
    Metrics GetFrameMetrics(const CMovie &movie, unsigned object, const ZMovieKeyFrame &frame,
    ...
    ZShaderProgram m_program, m_colorProgram;
    ZQuadBatch m_batch;
    ZMarkerBatch m_markers;
    ...
    std::map<std::uint64_t, std::unique_ptr<ZTexture>> m_gradients;
