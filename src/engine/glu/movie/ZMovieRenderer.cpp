...
CBitmapFont *ZMovieRenderer::GetFont(unsigned index) {
...
ZMovieRenderer::Animation *ZMovieRenderer::GetAnimation(unsigned archetype, unsigned animation) {
    const unsigned key = archetype * 256 + animation;
    auto found = m_animations.find(key);
    if (found != m_animations.end()) { return &found->second; }
    const ZSpriteArchetype *data = m_sprites.GetArchetype(static_cast<std::uint8_t>(archetype));
...
ZMovieKeyFrame ZMovieRenderer::AtTime(const CMovieObject &object, unsigned time) const {
    ZMovieKeyFrame result;
    ...
    const ZMovieKeyFrame &next = object.frames[current + 1];
...
ZMovieRenderer::Metrics ZMovieRenderer::GetMetrics(const CMovie &movie, unsigned objectIndex, unsigned time, float width, float height, unsigned depth) {
    if (depth > movie.objects.size() || objectIndex >= movie.objects.size()) { return {}; }
    const CMovieObject &object = movie.objects[objectIndex];
...
ZMovieRenderer::Metrics ZMovieRenderer::GetFrameMetrics(const CMovie &movie, unsigned objectIndex, const ZMovieKeyFrame &frame,
...
void ZMovieRenderer::Flush() { m_batch.Upload(); m_batch.Draw(m_program, m_projection); m_batch.Begin(); }

void ZMovieRenderer::Rectangle(float x, float y, float width, float height, float r, float g, float b, float alpha) {
    m_markers.Begin();
    m_markers.AddRect(x, y, width, height);
    m_markers.Draw(m_colorProgram, m_projection, r, g, b, alpha);
}

void ZMovieRenderer::Image(const ZTexture &texture, float x, float y, float width, float height, bool flipVertical) {
    ...
    m_batch.AddQuad(texture, x + (width - drawnWidth) / 2, y + (height - drawnHeight) / 2,
        drawnWidth, drawnHeight, source, false, flipVertical, ZBlendMode::Alpha);
    Flush();
}

bool ZMovieRenderer::Text(const std::string &text, float x, float y, unsigned font, float scale, float maxWidth, float alpha) {
...
float ZMovieRenderer::TextHeight(unsigned font, float scale) {
    CBitmapFont *bitmap = GetFont(font);
    if (bitmap == nullptr) { return 0; }
    return bitmap->Height(scale);
}

float ZMovieRenderer::TextWidth(const std::string &text, unsigned font, float scale) {
    CBitmapFont *value = GetFont(font);
    if (value == nullptr) { return 0; }
    return value->Width(text, scale);
}

bool ZMovieRenderer::SpriteFrameTimes(unsigned archetype, unsigned animation, std::vector<unsigned> &times) {
...
bool ZMovieRenderer::DrawSprite(unsigned archetype, unsigned animationIndex, unsigned time, float x, float y, float scale, float alpha, float rotation) {
    ...
    m_batch.Begin();
    for (const ZSpriteQuad &quad : animation->steps[step]) {
...
bool ZMovieRenderer::DrawSpriteFitted(unsigned archetype, unsigned animationIndex, unsigned time, float x, float y, float width, float height, float alpha) {
    Animation *animation = GetAnimation(archetype, animationIndex);
...
bool ZMovieRenderer::ButtonBackground(float x, float y, float width, float height, bool selected, bool hovered) {
...
bool ZMovieRenderer::StretchButtonSprite(unsigned animationIndex, float x, float y, float width, float height) {
    ...
    m_batch.Begin();
    for (const ZSpriteQuad &quad : animation->steps[0]) {
...
bool ZMovieRenderer::DrawNamed(const char *name, unsigned time, float x, float y) {
    const int ordinal = FindMovie(name);
    if (ordinal < 0) { return false; }
    return Draw(ordinal, time, x, y);
}

bool ZMovieRenderer::DrawFitted(unsigned ordinal, unsigned time, float x, float y, float width, float height, unsigned regionIndex) {
    for (const ZMovieRegion &region : Regions(ordinal, time)) {
...
bool ZMovieRenderer::Draw(unsigned ordinal, unsigned time, float x, float y, float width, float height, unsigned depth, float alpha,
    ZMovieRegionCallback *callback) {
    ...
    for (unsigned index : order) {
        ...
        if (object.type == 6 && callback != nullptr) {
            ...
            const ZMovieRegion region{regionIndex, frame.region, x + metrics.x, y + metrics.y,
        ...
        if (object.type == 7) {
            ...
            const ZSourceRect source{0, 0, 1, 256};
            m_batch.AddTransformedQuad(*m_gradients[key], x + metrics.x, y + metrics.y, metrics.width, metrics.height,
                source, false, false, ZBlendMode::Alpha, 0, 0, 1, 1, 0, frame.alpha * alpha);
            Flush();
            continue;
        }
        ...
        if (animation->duration != 0) {
            ...
            while (firstKey > 0) {
                const ZMovieKeyFrame &previous = object.frames[firstKey - 1];
            ...
            for (float tileY = phaseY; tileY < metrics.height; tileY += tileHeight) {
                for (float tileX = phaseX; tileX < metrics.width; tileX += tileWidth) {
                    for (const ZSpriteQuad &quad : animation->steps[step]) {
                        ...
                        ZSourceRect source = quad.source;
    ...
    Flush();
    if (m_regionOverlay) {
        for (const ZMovieRegion &region : Regions(ordinal, time, x, y)) {
            m_markers.Begin();
...
bool ZMovieRenderer::Gradient(float x, float y, float width, float height, unsigned topRgb, unsigned bottomRgb, float alpha) {
