...
ZMediaVideo::ZMediaVideo() : m_impl(new Impl()) {}
ZMediaVideo::~ZMediaVideo() = default;
unsigned ZMediaVideo::Width() const { return m_impl->width; }
unsigned ZMediaVideo::Height() const { return m_impl->height; }
std::uint64_t ZMediaVideo::DurationMs() const { return m_impl->durationMs; }

bool ZMediaVideo::Open(const std::filesystem::path &path) {
...
bool ZMediaVideo::ReadFrame(ZPNGImage &image, std::uint64_t &timestampMs, bool &ended) {
    ended = false;
    ComPtr<IMFSample> sample;
