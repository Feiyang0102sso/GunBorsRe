...
struct ZDecodedSound {
    SDL_AudioSpec specification;
    std::vector<std::uint8_t> samples;
};

struct ZPlayingSound {
...
struct ZAudioPlayer::Impl {
    std::map<std::uint64_t, ZDecodedSound> sounds;
    std::vector<ZPlayingSound> playing;
    std::vector<SDL_AudioStream *> idleStreams;
    SDL_AudioDeviceID device = 0;
    bool paused = false;
    std::uint64_t pauseStartMs = 0;
    float volume = ZAudioPlayer::GetEffectsGain();
