...
void ApplyBlendMode(ZBlendMode blend) {
    if (blend == ZBlendMode::Additive) {
        glBlendFunc(GL_SRC_ALPHA, GL_ONE);
        return;
    }
    if (blend == ZBlendMode::AdditiveOpaque) {
 ...
 * ends of each axis.
 */
void TexelCentreRect(const ZTexture &texture, const ZSourceRect &source,
...
bool ZQuadBatch::Create(const ZShaderProgram &program) {
    Destroy();
...
ZQuadBatch::Group &ZQuadBatch::GroupFor(const ZTexture &texture, ZBlendMode blend) {
...
void ZQuadBatch::AddQuad(const ZTexture &texture, float x, float y, float width,
                         float height, const ZSourceRect &source, bool flipHorizontal,
                         bool flipVertical, ZBlendMode blend) {
...
void ZQuadBatch::AddTransformedQuad(
    const ZTexture &texture, float x, float y, float width, float height,
    const ZSourceRect &source, bool flipHorizontal, bool flipVertical,
    ZBlendMode blend, float pivotX, float pivotY, float scaleX, float scaleY,
...
void ZQuadBatch::AddVertices(const ZTexture &texture, ZBlendMode blend,
...
void ZQuadBatch::AddGradientQuad(const ZTexture &color, const float *positions, const float *alpha) {
    ...
    AddVertices(color, ZBlendMode::AlphaColorFade, a, b, c, d);
}

void ZQuadBatch::Upload() {
    m_groupFirst.clear();
    m_groupCount.clear();
    m_groupTexture.clear();
    m_groupBlend.clear();
...
void ZQuadBatch::Draw(const ZShaderProgram &program, const float *mvp) const {
    ...
    for (std::size_t i = 0; i < m_groupFirst.size(); ++i) {
        ...
        glUniform1i(program.GetUniformLocation("additiveOpaque"),
                    m_groupBlend[i] == ZBlendMode::AdditiveOpaque);
        glUniform1i(program.GetUniformLocation("vertexColorFade"),
                    m_groupBlend[i] == ZBlendMode::AlphaColorFade);
    ...
    ApplyBlendMode(ZBlendMode::Alpha);
}

std::uint32_t ZQuadBatch::GetQuadCount() const {
