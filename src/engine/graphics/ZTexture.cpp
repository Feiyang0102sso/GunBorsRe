...
bool ZTexture::Create(const ZPNGImage &image, GLenum wrapMode) {
    Destroy();
...
void ZTexture::Bind(GLenum textureUnit) const {
    glActiveTexture(textureUnit);
    glBindTexture(GL_TEXTURE_2D, m_handle);
}

bool ZTexture::CaptureFramebuffer() {
    GLint viewport[4];
