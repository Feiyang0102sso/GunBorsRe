...
bool ZMarkerBatch::Create(const ZShaderProgram &program) {
    Destroy();
...
void ZMarkerBatch::Begin() {
    m_vertices.clear();
}

void ZMarkerBatch::AddRect(float x, float y, float width, float height) {
    const float left = x;
    const float top = y;
...
void ZMarkerBatch::AddOutline(float x, float y, float width, float height,
                              float thickness) {
...
void ZMarkerBatch::AddSegment(float firstX, float firstY, float secondX,
...
void ZMarkerBatch::Draw(const ZShaderProgram &program, const float *mvp,
