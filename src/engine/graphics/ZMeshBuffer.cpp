/**
 * @file ZMeshBuffer.cpp
 * @brief The GL buffers behind one 3D model.
 */

#include "engine/graphics/ZMeshBuffer.h"

#include "engine/graphics/ZQuadBatch.h"  // kTexCoordScale

#include <cstdio>
#include <vector>

ZMeshBuffer::ZMeshBuffer()
    : m_vertexArray(0),
      m_positionBuffer(0),
      m_texCoordBuffer(0),
      m_indexBuffer(0),
...
bool ZMeshBuffer::Create(const ZShaderProgram &program) {
    Destroy();
...
bool ZMeshBuffer::SetMesh(const CMesh &mesh) {
...
void ZMeshBuffer::SetFrame(const CMesh &mesh, std::size_t frameIndex) {
...
void ZMeshBuffer::SetVertices(const std::vector<float> &vertices) {
    if (vertices.empty()) {
        return;
    }
...
void ZMeshBuffer::Draw(const ZShaderProgram &program, const float *mvp,
                       const ZTexture &texture, float heatIntensity, const float *overlayRgb) const {
    if (m_indexCount == 0) {
        return;
    }
