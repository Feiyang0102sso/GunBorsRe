...
ZShaderProgram::ZShaderProgram() : m_program(0) {}

ZShaderProgram::~ZShaderProgram() {
    Destroy();
}

void ZShaderProgram::Destroy() {
    if (m_program != 0) {
...
GLuint ZShaderProgram::CompileStage(GLenum stageType, const std::string &path) const {
    std::string source;
...
bool ZShaderProgram::Load(const std::string &shaderDirectory,
...
GLint ZShaderProgram::GetUniformLocation(const char *name) const {
    if (m_program == 0) {
        return -1;
    }
...
GLint ZShaderProgram::GetAttribLocation(const char *name) const {
    if (m_program == 0) {
        return -1;
    }
