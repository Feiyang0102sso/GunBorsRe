...
- 当前职责与映射见 `_prep/docs/source-alignment-result.md`、`_prep/docs/source-name-map.md`。生存测试的选择与断言归 `tests/gameplay/SurvivalCheckScenario`；共享循环只提供资源、库存、生命周期和帧输入入口，不新增按测试项命名的运行回调。
