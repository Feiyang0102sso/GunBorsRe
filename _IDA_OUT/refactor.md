...
**按用户最新补充，debug key、cheatcode、cfg 的定义分别集中在各产品对应文件中，方便一次查找和管理。** 上表及目录树中的 DebugConfig.h 是上一版合并安排；是否拆为下述三个职责文件待本轮讨论确认。
...
已确定的前提：两个独立产品、共用引擎、产品控制允许重复、对应定义集中、全部使用 .h，以及 Release 编译排除测试／截图／作弊和调试绑定。以下选择互不依赖，可同一轮确认：

| 待定选择 | 建议 | 影响 |
|---|---|---|
| 对应文件如何命名和划分 | 两个产品各放 `DebugKeys.h`、`CheatCodes.h`、`Config.h`；分别登记调试键、作弊口令、cfg 键名／默认值及宿主设置。剩余开发默认值放 Config.h 的开发专用部分，替代原 DebugConfig.h，不新增目录。 | 查找入口固定，避免又长出万能配置头；Release 仍按能力宏排除开发部分。 |
...
术语见仓库根 [CONTEXT.md](../CONTEXT.md)。当前重复策略可逆且暂定，直接记录于本方案，不另建 ADR。上述建议等待本轮答复，尚未据此调整工程或实现代码。
