...
状态：优先重构主程序；已确认三个配置头文件及 tests/out 清理规则，查看器细节延后。当前仅更新方案，尚未迁移源码、改名 EXE 或落实 Release 编译隔离。
...
│   ├── DebugKeys.h                 游戏调试热键及动作登记
│   ├── CheatCodes.h                游戏作弊口令及命令登记
│   ├── Config.h                    cfg 键名、默认值及宿主配置
│   ├── gameplay/                   实体、战斗、地图、AI、关卡流程
│   ├── ui/                         菜单、HUD、动态绑定及转场
│   └── data/                       游戏条目、目录查询、进度及存档
├── gun_bros_viewer/                 查看器内容及启动器
│   ├── main.cpp                    查看器入口与命令路由
│   ├── DebugKeys.h                 查看器自己的调试热键
│   ├── CheatCodes.h                查看器自己的作弊／实验命令
│   ├── Config.h                    查看器自己的配置定义
│   ├── viewers/                    各类资源查看页
...
**用户已明确：最终仅保留主程序和 viewer 两个可执行工程。** 现有 research 是待整理的旧工具，不作为第三个产品或额外过渡程序保留。它的资源查看／研究入口并入 viewer，自动检查实现进入 tests；迁移完成后移除旧 research 工程、目录和 EXE 名称。共享静态库属于内部构建依赖，不增加可执行程序。
...
- engine 不依赖两个产品包；游戏内容不依赖 viewer。两个产品分别维护自己的热键、作弊命令及对应功能处理，即使完全重合也允许复制（暂定）；公共资源解析、播放和计算仍按上述分工调用共享实现，不通过包含生产 `.cpp` 访问内部代码。
...
两个 EXE 共用原 `big/`。当前 `gun_bros_re.exe`、`gun_bros_research.exe` 的工程输出、脚本、游戏内工具启动入口和说明分别迁移到 `GunBrosRe.exe`、`GunBrosViewer.exe`；更新调用方直接使用新名称，不保留 research 工程、旧名 EXE 或旧名称转发壳。查看器暂定名称统一由 Paths.h 定义，后续改名不散改调用点。
...
| 仓库 `gunbros.cfg` | 正式运行配置迁移到 `<exe-dir>/GunBrosRe.cfg`，开发开关转入对应配置头文件。 |
...
| `out/ui-original-*/`、`out/validation/`、`out/ui-movies/` 和其他固定测试输出 | 新产物统一到 `tests/out/`，绝对路径由测试驾驶器传入。历史产物原地保留。 |
...
测试产物根固定为仓库 `tests/out/`，不再采用上一版 `out/tests/<run-id>/`。测试脚本以自身位置定位仓库，将绝对输出路径传给每个检查；不以工作目录猜路径，不各自拼写带日期的 out 路径。每轮顶层测试运行前清空 tests/out，再按套件／用例建立子目录；一轮中的各用例不能重新清空整棵目录。

所有测试生成的日志、截图、报告、临时 cfg、存档副本和变异样本均在 tests/out 下；测试生成的其他缓存或转储也遵循此约定。`.gitignore` 显式登记 `/tests/out/`。执行前验证清理目标确实是本仓库的 tests/out，拒绝链接到目录外的清理目标；存在运行中的测试时拒绝再次清理。运行结束后保留本轮产物供排查，下次运行前再清空。这里只清理测试产物，不删除 tests 源码、原始输入或历史研究目录。

原 BIG／assets 可作为显式只读测试输入共用；需要变异的样本先复制到本轮实验目录。测试存档只能在本轮输出下，不能回退到 `<exe-dir>/saves/`、仓库 saves 或旧 userdata。现有 out 中的旧图片和研究索引本轮不清理；迁移后新测试产物全部遵守上述规则。若以后引入人工审核过的图片基准，它属于测试输入，应放在 tests/out 外，不能随运行产物一起清空。
...
- [test-muted.ps1](../test-muted.ps1) 按 Core、UI、OriginalUI 等组逐个启动现有研究 EXE，每项附加 `--mute`。通常退出码 0 表示通过；props 已知数据异常单独核对退出码及诊断文本，并标记 KnownDataIssue，不当作普通通过。脚本还记录耗时、标准输出、错误输出和 results.json，遇到失败停止后续检查。
- `Run*Check` 主要使用显式条件判断：检查资源能否读取、引用是否有效、状态是否变化、交互是否触发、账户读写是否一致；失败累计 failures 或提前返回非零。这些检查中既有数据检查，也有加载窗口并运行实际菜单／场景的集成检查。
- 存在运行内像素检查。例如 [SurvivalHud.cpp](../src/gun_bros_re/runtime/SurvivalHud.cpp) 验证按钮按下前后图像不同、保持按下时图像不变、松开后恢复；[MapScene.cpp](../src/gun_bros_re/runtime/MapScene.cpp) 包含现场绘制的遮挡参考比较。它们不是读取上轮 PNG 当标准图。
- SaveFrame 将帧缓存保存为 PNG，本身不执行图像正确性比较。当前检查范围内未发现统一的磁盘基准图对比系统；大量图片是供人检查与排错的证据。截图保存成功只能证明输出成功，不能证明 UI 正确。
- 脚本前后对仓库 saves 和 userdata 文件计算 SHA256，检查原始存档／实际账户是否被改动。目前报告写到 `out/validation/<时间戳>/`，图片与临时存档由 C++ 各自指定 out 路径，脚本将工作目录设为仓库根；没有统一运行前清理。

### 测试输出流程与框架选择

新流程为：校验测试输入与程序 → 确认没有上一轮进程运行 → 清空并重建 tests/out → 逐项运行 → 写入结果汇总及退出码。失败时保留已产生的日志、截图和临时存档，下次顶层运行前再清理。不能通过改变工作目录让旧路径“碰巧正确”；须把所有输出点接入明确的测试输出路径。

```text
tests/
├── MenuChecks.cpp                  测试源码，正常纳入版本管理
├── SceneChecks.cpp
├── ResourceChecks.cpp
├── ProfileChecks.cpp
└── out/                            全部忽略提交，每轮开始前清空
...
是否引入 gtest 尚未定案。建议第一步保留现有检查判定，先完成主程序与 tests 的分离、输出路径和 Release 隔离，避免目录重构同时重写测试。检查函数可继续返回退出码，由开发版 viewer 的命令入口调用；不另设 research 或独立测试 EXE。先将旧工具必要入口整理为 viewer 的最小入口即可，不要求先完成查看器界面与全部功能重构。

后续若需要更细的测试定位，可将不依赖窗口的资源解析、脚本、状态转换和存档逻辑逐步接入 GoogleTest。它提供测试组织、断言和测试夹具，失败时能报告具体断言；图像对比规则、测试输出路径及运行前清理仍需项目定义，采用框架并不能自动解决这些问题。参见 [GoogleTest 官方入门](https://google.github.io/googletest/primer.html)。引入框架不改变两个发行产品，也不把框架链接进游戏 Release。
...
以上数量为前次调研快照，后续新增检查须在实施时重新登记。**现有检查和截图能力保留，旧图片本轮不删除**。迁移后所有新测试产物统一写到 `tests/out/<suite>/<case>/`，遵循运行前清空、运行后保留的规则。普通启动不自动生成图片；截图只由开发构建中明确运行的测试／截图命令触发。
...
|---|---|---|---|
| Debug | 显式启用，由开发版 viewer 调用 tests；不另设 research 程序 | 采用各产品 DebugKeys.h、CheatCodes.h 及 Config.h 的开发配置 | 保留 |
...
- 构建配置集中定义能力宏，如 `GB_ENABLE_TESTS`、`GB_ENABLE_CAPTURE`、`GB_ENABLE_CHEATS`，Release 固定为 0；开发绑定表、少量开发接口的声明和实现一起受控。各配置头文件不得覆盖 Release 的禁用约束，也不靠 constexpr false 代替编译排除。
...
| 各产品的 `DebugKeys.h` | 调试热键、动作名称及用途说明 | 仅 Debug／Validation 使用，Release 排除开发表及处理器。 |
| 各产品的 `CheatCodes.h` | 作弊口令、命令名称及用途说明 | 仅 Debug／Validation 使用，Release 排除作弊入口。 |
| 各产品的 `Config.h` | cfg 键名、默认值及宿主配置；其余开发默认值单独分区 | 正式配置可用于 Release；开发专用部分按能力宏排除。 |

**用户已确认分别使用 DebugKeys.h、CheatCodes.h、Config.h**，替代上一版合并的 DebugConfig.h；两个产品各自拥有这三个文件，不新增配置目录。主程序先落实，查看器在后续阶段落实。

两个产品独立维护自己的绑定、命令及对应功能处理，**即使 100% 重合也复制一份（暂定）**。不建立跨产品的热键／作弊注册表，不让 viewer 为复用这些功能而依赖游戏启动器。后续修改一方不自动同步另一方，需要同步时分别修改。此约定替代上一版“不重复维护同一组常量”的要求；公共引擎能力继续共用，引擎不依赖产品命令。

“集中”包括键名／口令、默认值、动作名称及简短用途说明；输入循环和各页面只消费定义，不再散写第二份键码、口令或 cfg 键名。功能实现归各产品对应源文件，不把大量操作逻辑塞进配置头文件。共用路径仍统一引用 Paths.h。
...
已确定两个产品和三个配置头文件。以下两项按用户要求延后，不作为主程序重构的前置条件；表内建议尚未获确认：

| 待定选择 | 建议 | 影响 |
|---|---|---|
| 查看器是否独立保存设置 | 如需持久化窗口大小、上次查看资源等查看器状态，使用同目录 `GunBrosViewer.cfg`，不读取或改写 GunBrosRe.cfg；不需要持久化则不创建。 | 确认后再更新发行目录树和允许写入清单。 |
| 调试键何时响应 | 绑定表同时列出适用页面及触发方式；普通切换命令按下一次执行一次，文本输入期间屏蔽调试键。作弊口令使用明确输入入口。 | 避免搜索资源时误触暂停／重置，或按住按键连续执行命令；具体入口随产品分别维护。 |

术语见仓库根 [CONTEXT.md](../CONTEXT.md)。当前重复策略可逆且暂定，直接记录于本方案，不另建 ADR。上述查看器建议暂不实施，也不在本轮继续要求用户选择。

## 11. 执行顺序与验收

**第一轮重构以主程序为目标，查看器完整整理后做。** 抽离主程序依赖的 engine 和 tests 属于本轮必要工作；旧 research 仅做并入 viewer 所需的入口／工程调整，以保留回归检查，不另留过渡工程或可执行程序。查看器界面、cfg 及热键响应规则仍延后。下面的代码迁移任务尚未执行。

顺序原则：先让回归检查可重复运行，再隔离测试和开发功能，然后从底层往上迁移；菜单和场景等长文件在依赖稳定后拆。每一阶段通过对应验收才勾选，不能把“文件已移动”当作完成。以下均为待执行任务；阶段内只处理一个模块，避免同时重排全工程。

### 1. 登记基线，统一测试输出

- [ ] 登记当前构建命令、全部检查／milestone 入口和已知失败；重新清点源码，不沿用旧数量作为验收标准。
- [ ] 先接通 `tests/out/` 的绝对路径传递和顶层运行前清理，将日志、截图、报告、临时 cfg、存档及缓存等输出点逐项迁入；检查用例之间不会互删产物。
- [ ] 使用现有检查建立基线，并验证进入关卡、移动射击、受伤死亡、重开和进度重载；记录命令、退出码及必要画面。已知问题独立登记，不悄悄当作通过。

验收：连续跑两轮时，第二轮没有第一轮残留；所有新测试产物都在 tests/out，原始样本与玩家账户未被改动。需保留的基线结论写入文档，必要证据明确保存到输出清理范围外。

### 2. 收敛为两个程序，保留可运行入口

- [ ] 将输出名统一为 `GunBrosRe.exe`、暂定 `GunBrosViewer.exe`，把旧 research 工程与入口整理为 viewer 的最小入口。
- [ ] 更新脚本、解决方案、工程引用和工具启动路径，移除 research 旧工程／名称与转发壳；核对旧工具入口仍可访问。

验收：只有两个可执行工程，正式游戏能启动，开发版 viewer 能调用现有检查。此阶段只接入口，不设计查看器新界面，也不提前决定其 cfg 和热键响应规则。

### 3. 从主程序抽出 tests

- [ ] 逐文件迁出 `Run*Check`，再清理嵌在正式流程中的自动输入、固定实验控制和检查分支；覆盖前面登记的全部入口。
- [ ] 把截图、帧像素检查和测试辅助实现集中到 tests；需要共享的正常业务逻辑留在生产模块，检查通过明确接口调用，不包含生产 `.cpp`。
- [ ] 改由开发版 viewer 调用 tests，游戏公共头移除检查接口；建议补齐失败位置、预期值和实际值等诊断，不借此整体重写测试或引入框架。

验收：原有检查逐项回归，失败能定位；游戏公共接口不再携带测试参数。发现确有业务耦合时只拆开必要接口，其余长文件拆分留到后续阶段。

### 4. 集中主程序配置，落实 Release 隔离

- [ ] 在 gun_bros_re 中建立 `DebugKeys.h`、`CheatCodes.h`、`Config.h`，逐项归集定义与用途说明；调试键与正式操作分开。
- [ ] 集中构建能力开关，使游戏 Release 排除 tests、截图、作弊和调试绑定／处理器；保留正式日志及正常输入。
- [ ] 检查编译项、链接依赖及运行入口；确认 cfg、命令行和按键不能重新启用被排除的开发功能。

验收：游戏 Debug 回归可用，Release 无开发入口；不依赖 `NDEBUG` 关闭普通 assert 来冒充测试代码隔离。需要优化构建检查时使用独立 Validation 配置。

### 5. 抽取 engine 基础依赖

- [ ] 按依赖顺序迁移基础结构／数学、流与 BIG／TOC、公共资源类型；消除引擎反向包含游戏类型的关系。
- [ ] 迁移窗口、输入、音视频适配，以及纹理、字体、模型、骨骼与绘制能力；每类迁移后运行对应检查。
- [ ] 建立共享静态库引用，两个程序使用同一引擎实现；如 viewer 需要游戏规则，引用游戏内容库，不链接游戏 main。

验收：引擎不依赖产品包，同配置公共引擎不重复编译；资源读取、字体、模型、音视频和正常游戏启动保持基线行为。

### 6. 整理 engine/glu

- [ ] 迁移 Script 至 `engine/glu/script/`、SpriteGlu 至 `engine/glu/sprite/`。
- [ ] 将 CMovie、MovieRenderer 的资源、播放与绘制职责整理到 `engine/glu/movie/`；保持游戏原生函数和动态区域绑定由产品提供。
- [ ] 回归脚本执行、图集动画、Movie 章节、时间、显隐、区域和完成事件；记录已有文字／声音／嵌套行为缺口。

验收：两个程序对相同资源得到一致的解析与播放结果。目录迁移不顺手改变原版行为或补造未知数据，行为修复另列任务。

### 7. 整理主程序数据与启动职责

- [ ] 将 Catalog、条目查询、进度和存档归入 `gun_bros_re/data/`，拆清 PackTables 的包级读取与游戏语义。
- [ ] 将启动顺序、宿主设置和装配留在游戏包，main 只负责初始化和进入流程。
- [ ] 核对存档往返、装备／进度读取及游戏原生调用，保持原 BIG 与客户端规则。

验收：数据与引擎职责清楚，启动、账户加载和进度保存保持基线行为；原存档样本没有被当作玩家可写目录。

### 8. 拆主程序长文件，逐个收尾

- [ ] 先拆 GameFrontEnd：一次处理一个菜单，将其状态、输入、更新和绘制放在一起；页面切换留给流程控制。
- [ ] 再拆 SurvivalHud：按 HUD、通知、暂停及道具选择等职责组织，同目录优先，不为每个类增加文件夹。
- [ ] 最后拆 MapScene：区分地图实例、战斗／AI、关卡流程和绘制调用，资源查看归 viewer，实验驾驶归 tests。
- [ ] 核对其他 runtime 文件归属，更新 include、工程项、过滤器与说明后取消混合 runtime 目录。

验收：每拆一个页面或场景职责就跑相应回归；最后验证进入地图、移动射击、刷怪、受伤死亡、重开、结算和进度重载。不要把三个大文件一次拆完再测试。

### 9. 落实发行路径与主程序打包

- [ ] 在 `engine/core/Paths.h` 集中目录与产品名称约定，平台层以 EXE 所在目录解析实际路径；统一 cfg、日志、资源及存档调用点。
- [ ] 在独立暂存目录装配 `GunBrosRe.exe`、`GunBrosRe.cfg`、big、saves、logs、assets 及必要运行库，移除 ASSET_ROOT 和源码树兜底。
- [ ] 在不同工作目录、含空格／中文的新位置测试发行包；不安装 viewer 时验证正式游戏闭环与保存重载。

验收：正式写入仅在已约定的 cfg、saves、logs；Release 不产生测试图片，不需要源码、研究目录或旧 research 程序。测试输出仍全部进入 tests/out。

### 10. 最后完整整理 viewer

- [ ] 整理资源查看界面及 milestone 路由，计算继续调用 engine；保留已登记的查看／研究能力。
- [ ] 独立维护 viewer 的 DebugKeys.h、CheatCodes.h、Config.h，允许与游戏控制重复；此时再讨论其 cfg 和输入响应规则。
- [ ] 验证开发版可调用 tests、Release 排除测试／截图／作弊，且 viewer 不修改正式账户。

验收：两个程序各自可运行、职责清楚；不恢复第三个 research 或测试 EXE。
