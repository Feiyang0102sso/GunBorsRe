...
- “每种手雷都会掉一层甲”不成立。当前 3.6.0 资源中，冰雷先走属性 2 分支，电雷再走属性 3 分支，两者都不会递减装甲计数；只有带属性 4、且不带属性 2/3 的普通破片雷进入拆甲分支。
...
2. Boss Flow 使用 `CEnemy.native_0x0731(index)` 检查碰撞属性。冰属性 2 优先，随后检查手雷属性 4；普通破片雷的拆甲函数还会排除电属性 3。
...
这与 Boss Flow 的判断顺序吻合：冰雷进入冻结/控制分支，电雷进入电击/控制分支，普通破片雷才递减装甲。旧版 `flow scripts/game.link` 没有列出属性 4，版本早于当前资源，只能作为辅助，不可覆盖当前 BIG 和反编译消费者证据。
