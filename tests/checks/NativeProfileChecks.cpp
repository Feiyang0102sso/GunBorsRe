...
int RunNativeProfileCheck(const std::string &bigDirectory) {
    ...
    CGunBros tables(toc);
    CProfileManager::Archive defaults;
    if (!CProfileManager::CreateArchive(toc, tables, defaults)) { return 1; }
    const std::filesystem::path root = TestOutput::Path("ui-original-2026-09-09/native-profile");
    const auto empty = root / "new";
    if (!CProfileManager::WriteArchive(empty, defaults)) { return 1; }
    CProfileManager::Archive imported = defaults;
    ...
    for (unsigned id = kFirstStore; id <= kLastStore; ++id) {
        ...
        CProfileManager::Record restored;
        if (!CProfileManager::ReadRecord(empty / filename, id, restored) || restored.payload != defaults.records[id - kFirstStore].payload ||
            !CProfileManager::ReadRecord(source / filename, id, imported.records[id - kFirstStore])) { return 1; }
        ++count;
    }
    std::vector<std::uint8_t> status;
    if (!ZProfileFiles::Read(source / "-1_PDST", status) || status.size() != imported.status.size()) { return 1; }
    std::copy(status.begin(), status.end(), imported.status.begin());
    if (!CProfileManager::WriteArchive(root / "imported", imported)) { return 1; }
    for (unsigned id = kFirstStore; id <= kLastStore; ++id) {
        if (id == kUnregisteredStore) { continue; }
        std::vector<std::uint8_t> written;
        if (!ZProfileFiles::Read(root / "imported" / ("-1_" + std::to_string(id)), written) ||
    ...
    CProfileManager::Record preserved = imported.records[0];
    auto broken = preserved.originalFile;
    broken.back() ^= 1;
    if (!ZProfileFiles::Replace(root / "bad-crc", broken) || CProfileManager::ReadRecord(root / "bad-crc", 1000, preserved) ||
    ...
    for (const CStoreItem::Entry &entry : store) {
        ...
        if (buyer.AcquireItem(entry.data, entry.data.requiredLevel) != CProfileManager::PurchaseResult::Purchased ||
        ...
        if (buyer.AcquireItem(entry.data, entry.data.requiredLevel) != CProfileManager::PurchaseResult::Owned ||
    ...
    if (!(fresh).LoadNative(toc, tables, freshPath, root / "absent-source") || !fresh.firstLaunch ||
    ...
    if (!ZProfileFiles::Read(freshPath / "p", optionBytes) || optionBytes.size() != 36 ||
        Get32(optionBytes, 0) != CCrc32::Crc32(optionBytes.data() + 4, 32)) { return 1; }
    optionBytes[0] ^= 1;
    if (!ZProfileFiles::Replace(freshPath / "p", optionBytes) || fresh.LoadFromDisk(freshPath) ||
    ...
    for (const auto &planet : planets) {
        for (unsigned index = 0; index < planet.missions.size(); ++index) {
            ...
            if (!(fresh).RecordMissionWaves(mission.level, mission.value64 + 2, {true, false}) ||
                !(fresh).RecordMissionScore(ref, 24000) || !(fresh).RecordMissionScore(ref, 12000) ||
