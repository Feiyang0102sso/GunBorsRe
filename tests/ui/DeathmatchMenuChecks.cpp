...
int CheckDeathmatchMenus(CResTOCManager &toc, PackTables &tables, CProfileManager &profile) {
    GameMenu view;
    ...
    view.Begin(0);
    view.SetTestClick({mode.x + mode.width / 2, mode.y + mode.height / 2});
    if (!DrawOriginalModeOverlay(view, match) || match.gameMode != 2) { return 1; }
    ...
    std::vector<MenuTestClick> waits(12, {-100, -100, 500});
    ...
    state.originalUi = true; state.deathmatch = true; state.withBrother = true;
    ...
    view.Begin(0);
    if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-intro.png"))) { return 1; }
    ...
    hud.ResetNotices();
    view.Begin(0);
    if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-respawn.png"))) { return 1; }
    MovieRegion pause;
    if (!hud.FindActionRegion(state, SurvivalHudAction::Pause, pause)) { return 1; }
    hud.Pointer(state, pause.x + pause.width / 2, pause.y + pause.height / 2, false);
    if (hud.Pointer(state, pause.x + pause.width / 2, pause.y + pause.height / 2, true) != SurvivalHudAction::Pause) { return 1; }
    state.paused = true;
    view.Begin(0);
    ...
    view.Begin(0);
    MovieRegion initialGun;
    if (!hud.Draw(state) || !hud.FindActionRegion(state, SurvivalHudAction::SelectMatchGun, initialGun)) {
        std::printf("[dm-entry-selector] initial shop did not open GUNS\n");
        return 1;
    }
    if (!GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-initial-guns.png"))) { return 1; }
    hud.ResetSelector();
    view.Begin(0);
    if (!hud.Draw(state)) { return 1; }
    hud.AdvanceMenu(3000);
    view.Begin(0);
    if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-shop.png"))) { return 1; }
    MovieRegion powerupTab, gunTab, powerupGraphic, gunGraphic, powerupTouch, gunTouch;
    const auto *powerupButton = OriginalMenuData("MDS_BUTTON_POWERUP_SELECTOR", 4);
    const auto *gunButton = OriginalMenuData("MDS_BUTTON_POWERUP_SELECTOR", 5);
    if (!hud.FindActionRegion(state, SurvivalHudAction::ShowPowerups, powerupTab) ||
        !hud.FindActionRegion(state, SurvivalHudAction::ShowGuns, gunTab) ||
    ...
    const bool defaultPowerups = !hud.FindActionRegion(state, SurvivalHudAction::SelectMatchGun, gunGraphic);
    ...
    view.Begin(0);
    if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-guns.png"))) { return 1; }
    MovieRegion gunCard;
    if (!hud.FindActionRegion(state, SurvivalHudAction::SelectMatchGun, gunCard)) { return 1; }
    ...
    if (hud.Pointer(state, gunCard.x + gunCard.width / 2, gunCard.y + gunCard.height / 2, false) != SurvivalHudAction::SelectMatchGun) { return 1; }
    hud.AdvanceMatchSelection();
    hud.AdvanceMenu(250);
    view.Begin(0);
    if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-gun-selected.png"))) { return 1; }
    // A death may replace an already-open gun shop without a hidden render frame.
    hud.ResetSelector();
    view.Begin(0);
    if (!hud.Draw(state) || hud.m_matchGuns) { return 1; }
    ...
    for (const auto &entry : powerups) {
        ...
        view.Begin(0);
        if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-cooldown.png"))) { return 1; }
        cooldownDrawn = true;
    }
    ...
    view.Begin(0);
    if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-powerup-message.png"))) { return 1; }
    ...
    if (!hud.Draw(state) || !GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-fade.png"))) { return 1; }
    ...
    SurvivalGameContext context{profile, {}};
    ...
    if (results.page != 27 || results.postGame.postGameUpgradePending) { return 1; }
    for (unsigned frame = 0; frame < 300; ++frame) {
        view.clock += 16;
        view.Begin(27);
        if (!DrawOriginalPostGame(view, results, toc, tables, profile)) { return 1; }
    }
    if (!GB_SAVE_FRAME(view.window, TestOutput::Path("deathmatch-results.png"))) { return 1; }
    results.postGame.livePosition = 4;
    view.Begin(27);
    if (!DrawOriginalPostGame(view, results, toc, tables, profile) || results.postGame.livePosition != 3) { return 1; }
    MovieRegion replay;
