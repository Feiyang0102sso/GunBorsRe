...
int CheckDeathmatchMenus(CResTOCManager &toc, ZPackTables &tables, CProfileManager &profile) {
    ...
    ZMenuState match;
    view.Begin(0);
    if (!DrawModeOverlay(view, match)) { return 1; }
    ZMovieRegion mode;
    ...
    if (!DrawModeOverlay(view, match) || match.gameMode != 2) { return 1; }
    ...
    const auto *powerupButton = FindMenuData("MDS_BUTTON_POWERUP_SELECTOR", 4);
    const auto *gunButton = FindMenuData("MDS_BUTTON_POWERUP_SELECTOR", 5);
    ...
    view.Begin(27);
    if (!DrawPostGame(view, results, toc, tables, profile) || results.postGame.livePosition != 3) { return 1; }
    ZMovieRegion replay;
