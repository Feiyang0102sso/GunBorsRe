...
bool FinishRefineryClick(ZGameMenu &view, ZMenuState &state, CProfileManager &profile,
    ...
    if (duration > 1) {
        ...
        view.Begin(3);
        if (!DrawRefinery(view, state, profile, data, path, now) || !state.refinery.chambers[slot].clickPlaying ||
            state.refinery.refineryTransfer >= 0 || profile.xplodium != ore || profile.coins != coins) { return false; }
    }
    ++view.clock;
    view.Begin(3);
    return DrawRefinery(view, state, profile, data, path, now) && !state.refinery.chambers[slot].clickPlaying &&
...
int CheckOnlineRefinery(CResTOCManager &toc, ZPackTables &tables, ZGameMenu &view,
    ...
    if (!DrawRefinery(view, state, profile, data, path, now) || !state.storePromptRequested ||
    ...
    view.clock += 375;
    view.Begin(3);
    if (!DrawRefinery(view, state, profile, data, path, now) || profile.refinery.slots[slot].state != 2 || profile.xplodium) { return 1; }
    ...
    view.Begin(3);
    view.SetTestClick({x, y});
    if (!DrawRefinery(view, state, profile, data, path, middle) || state.refinery.chambers[slot].clickTime != pulseStart + 200) { return 1; }
    view.clock += (pulseEnd - pulseStart + 1) / 2;
    view.Begin(3);
    if (!DrawRefinery(view, state, profile, data, path, middle) || state.refinery.chambers[slot].clickPlaying) { return 1; }
    view.SetTestClick({x, y});
    if (!DrawRefinery(view, state, profile, data, path, middle) || !state.refinery.chambers[slot].clickPlaying) { return 1; }
    GameHostSettings().isConnected = false;
    view.Begin(3);
    if (!DrawRefinery(view, state, profile, data, path, middle) || state.refinery.refineryStatusChapter[slot] != 0) { return 1; }
    ...
    view.Begin(3);
    if (!DrawRefinery(view, state, profile, data, path, finish) || state.refinery.refineryStatusChapter[slot] != 3) { return 1; }
    const auto yield = profile.refinery.GetRefinementSlotYield(slot);
    view.Begin(3);
    view.SetTestClick({x, y});
    if (!DrawRefinery(view, state, profile, data, path, finish) ||
