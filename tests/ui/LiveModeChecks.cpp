...
std::vector<std::uint8_t> ReadLiveListPixels(GameMenu &view, MovieRegion region) {
...
int CheckLocalRoster(CResTOCManager &toc, PackTables &tables, CProfileManager &profile, GameMenu &view) {
    const auto root = std::filesystem::path(TestOutput::Path("roster"));
    CProfileManager player = profile;
    LocalBotFriend migrated;
    ...
    view.Begin(4);
    if (!DrawOriginalSocialMenu(view, friends, player, false) || !friends.social.onlinePage ||
        !GB_SAVE_FRAME(view.window, TestOutput::Path("local-bot-roster.png"))) { return 1; }
    friends.social.socialTab = 1;
    view.Begin(4);
    if (!DrawOriginalSocialMenu(view, friends, player, false) ||
        !GB_SAVE_FRAME(view.window, TestOutput::Path("local-bot-boost.png"))) { return 1; }
    ...
    GameHostSettings().isConnected = true;
    view.Begin(4);
    if (!DrawOriginalSocialMenu(view, compact, player, false)) { return 1; }
    ...
    if (!glow || compact.social.brotherName != view.movies.NamedString("IDS_FRIEND_DEFAULT_BRO2") ||
        !GB_SAVE_FRAME(view.window, TestOutput::Path("bros-selected-glow.png"))) { return 1; }
    view.Begin(4);
    view.clock += 16;
    view.SetTestClick({viewport.x + 10, viewport.y + 10});
    view.pointerPressed = view.pointerHeld = true;
    view.dragY = -300;
    const bool drawn = DrawOriginalSocialMenu(view, compact, player, false);
    ...
    compact.social.scrollMotion = MenuScrollMotion{};
    ...
    view.Begin(4);
    view.SetTestClick({lastRow.x + lastCard.x - 512 + lastCard.width / 2, lastRow.y + lastCard.y - 384 + lastCard.height / 2});
    if (!DrawOriginalSocialMenu(view, compact, player, false) || compact.social.selectedLocalFriend != 3) { return 1; }
    view.clock += 200;
    view.Begin(4);
    if (!DrawOriginalSocialMenu(view, compact, player, false) ||
        !GB_SAVE_FRAME(view.window, TestOutput::Path("bros-last-selected.png"))) { return 1; }
    GameHostSettings().isConnected = wasConnected;
...
int CheckLiveMode(CResTOCManager &toc, PackTables &tables, CProfileManager &profile) {
    ...
    MultiplayerStatistics player, peer;
    ...
    view.Begin(0);
    if (!hud.Draw(hudState) || !GB_SAVE_FRAME(view.window, TestOutput::Path("live-wave-results.png"))) { return 1; }
    while (hud.LiveWaveRemaining() > 6000) { hud.Advance(1); }
    view.Begin(0);
    if (!hud.Draw(hudState) || !GB_SAVE_FRAME(view.window, TestOutput::Path("live-wave-countdown.png"))) { return 1; }
    hudState.indicators.push_back({-1, kBrotherCombatId, 4, 512, -100, 500, -1});
    view.Begin(0);
    if (!hud.Draw(hudState) || !GB_SAVE_FRAME(view.window, TestOutput::Path("live-peer-indicator.png"))) { return 1; }
    ...
    view.Begin(0);
    if (!hud.Draw(hudState) || !GB_SAVE_FRAME(view.window, TestOutput::Path("live-peer-shop.png"))) { return 1; }
    for (unsigned y = 0; y < 768; y += 32) {
        for (unsigned x = 0; x < 1024; x += 32) {
            if (hud.Pointer(hudState, float(x), float(y), true) != SurvivalHudAction::None) { return 1; }
    ...
    if (!GB_SAVE_FRAME(view.window, TestOutput::Path("live-final-results.png"))) { return 1; }
    ...
    view.Begin(27);
    if (!DrawOriginalPostGame(view, results, toc, tables, profile) ||
        !GB_SAVE_FRAME(view.window, TestOutput::Path("live-final-results-bottom.png"))) { return 1; }
    MovieRegion replay;
